...
<!--        <text id='test_desc'></text>-->
        ...
        <div id="reservation_container" class="container" style="margin-top:50px;display:none;background-color:#ffffff">
            ...
            <div id="reservation_center" class="reservation_center">
                ...
                <label id='label_title' style="color:gray;font-weight:bold;padding:5px;">전체 예약표</label>
                
                <div id='div_month_total_table' style ='height:auto;background-color:#fff7d1'>
...
<!--                                <td><text id="month_all_user" style='font-weight:bold;color:#555555'>0</text>&nbsp;명</td>-->
                                <td style="background-image:linear-gradient(to bottom,  #fff7d1 0px, #dfd7b1 100%);"><text id="month_now_user" style='font-weight:bold;color:#555555'>0</text>&nbsp;명</td>
                                <td style="background-image:linear-gradient(to bottom,  #fff7d1 0px, #dfd7b1 100%);width:1px"></td>
                                <td style="background-image:linear-gradient(to bottom,  #fff7d1 0px, #dfd7b1 100%);"><text id="month_all_reservation_count" style='font-weight:bold;color:#555555'>0</text>&nbsp;회</td>
                                <td style='background-image:linear-gradient(to bottom,  #996799 0px, #aa89aa 100%)'><text id="month_now_reservation_count" style='font-weight:bold;color:blue'>0</text>&nbsp;회</td>
<!--                                <td><text id="month_now_reservation_finish_count" style='font-weight:bold;color:blue'>0</text>&nbsp;회</td>-->
                                <td style="background-image:linear-gradient(to bottom,  #fff7d1 0px, #dfd7b1 100%);max-width:10px"></td>
                                <td style='background-image:linear-gradient(to bottom, #dd8818 0px, #ffaa2a 100%)'><text id="month_total_confirm_count" style='font-weight:bold;color:#dd0000'>0</text>&nbsp;회</td>
                ...
                <div id='div_user_coupon' style="height:auto;">
                    <table class="table table-bordered"  id="user_coupon_table" cellspacing="0">
                        <thead>
                            <tr align="center"  style="background-color:#eeeeee">
                                <td class='textevent' style="font-size:12px;">인덱스</td>
                                <td class='textevent' style="font-size:12px;">결제ID</td><!--couponid-->
                                <td class='textevent' style="font-size:12px;">기간</td>
                                <td class='textevent' style="font-size:12px;">등록횟수</td>
                                <td class='textevent' style="font-size:12px;">완료/확인/신청</td>
                                <td class='textevent' style="font-size:12px;">목록보기</td>
                            </tr>
...
<!--    <script>TouchEmulator()</script>-->
    ...
    <script>
        ...
        console.log("d_teacher_reservation!!");
...
//            mainElement: '#main',
//            onRefresh: function() { 
////                var mode = window.options.mode;
////                var date = window.options.date.getTime();
////                param_day = mode == "day" ? date : "";
////                refresh_page("?centercode="+param_centercode+"&day="+param_day); 
//                reload_calendar_page();
//            }
//        });
//        document.getElementById("test_desc").innerHTML = location.href;
...
//        var combobox  = createCustomComboBox("testid","==테스트선택==",objs,1.5);
//        var default_info = document.getElementById("testdiv");
...
//        2)
        //커스텀콤보박스 테스트         
        //var objs = [{"value":"v1","text":"셀렉트1","imgname":"./img/arrow_l.png"},{"value":"v2","text":"셀렉트2","imgname":"./img/arrow_r.png"},{"value":"v3","text":"셀렉트3","imgname":"./img/black_arc_logo.png"}];
//        C_ComboBox("span_top_right","testid","==test==",objs,1,"aaa",-1);
        ///////////////////////////////////////
...
//                    clog("mem_teacher_alldata ",mem_teacher_alldata);
        ...
        var gxtypes = null;
        var gxteachers = null;
        function GX_getTeacher(){
            
       
            console.log("11 param_centercode is "+param_centercode);
...
//        var weeks = getThisWeek();
        console.log("insertGXCalenderDatas 00");
        if(datas)
        for(i = 0; i < datas.length; i++) {
            var roomdata = JSON.parse(datas[i].gx_roomdata);
            console.log("insertGXCalenderDatas 11");
...
//                var date = getIOSDate(tdate);
              ...
              console.log("insertGXCalenderDatas 22");
                ...
                if(roomdata.room_managerid == param_teacheruid){
                    console.log("insertGXCalenderDatas 33");
                    if(!totalgxroomcount[ym])totalgxroomcount[ym] = 0;
                        totalgxroomcount[ym]++;    
...
//            clog(" scale "+zoom);
            ...
            if(user_uid != ""){
                ...
                }else div_user_coupon.style.display = "none";
            }else{
                //hide
                div_user_coupon.style.display = "none";
            }
        }
//        function getMaxCount(user_uid, couponid){
//            var maxcount = 0;
//            for(var i = 0 ;i < mem_teacher_alldata[0].myusers.length;i++){
//                var user = mem_teacher_alldata[0].myusers[i];
//                if(user.uid = user_uid && user.couponid == couponid){
//                    maxcount = user.maxcount ? parseInt(user.maxcount) : 0;
//                    break;
//                }
//            }
//            return maxcount;
//        }
        function getMaxCount(myuser){
            var issendedcount = 0;
            for(var i = 0; i < newuserinfos.length;i++){
                if(myuser.uid == newuserinfos[i].uid && myuser.couponid == newuserinfos[i].id && newuserinfos[i].issendedcoupon && parseInt(newuserinfos[i].issendedcoupon) > 0){
                    issendedcount = parseInt(newuserinfos[i].issendedcoupon);
                    break;
                }
            }
            var max_count = issendedcount > 0 ? issendedcount : parseInt(myuser.maxcount);
...
//            clog("rows ",rows);
            ...
            for(key in couponlist){
                ...
                var isdelete = myuser.isdelete ? myuser.isdelete : "N";
                var finish_cnt = 0;   //완료된 횟수
                var confirm_cnt = 0; //고객확인된 횟수
                var request_cnt = 0; //신청만한 횟수
                var maxcount = getMaxCount(myuser);
                ...
                if (len > 0) {
                    ...
                    brow.style.backgroundImage = finish_cnt == totalmaxcount || getDDay(myuser.endtime) < 0  ? "linear-gradient(to bottom,  #eee 0px, #ddd 100%)" : "linear-gradient(to bottom,  #fff7d1 0px, #dfd7b1 100%)";
                    brow.style.textAlign = "center";
                    brow.style.height = "auto";

                    // 인덱스
                    var bcell_index = brow.insertCell();
                    bcell_index.innerHTML = cnt+"";
                    ...
                    bcell_id.innerHTML = "<text style='color:white;text-shadow:1px 1px 1px #f00;'>["+mmonth+""+mday+"]</text>";
                    ...
                    bcell_total.innerHTML = freecount > 0 ? "<text style='color:red;font-size:13px'>[무료"+freecount+"회] + </text>"+maxcount+"회" : maxcount+"회";
                    bcell_total.style.minWidth = "150px";

                    //PT완료/고객승인/운동신청
                    var bcell_finish = brow.insertCell();
                    bcell_finish.innerHTML = finish_cnt+" / "+confirm_cnt+" / "+request_cnt;
                    bcell_finish.style.minWidth = "150px";

                    //목록보기 버튼
                    var bcell_confirm = brow.insertCell();
                    var str_rows = setJSONStringToParamString(ptlist);
                    bcell_confirm.innerHTML = "<button class='btn btn-primary' style='float:right;margin-top:-9px;' onclick='showCouponList( \"" + couponid + "\", "+totalmaxcount+", \"" + str_rows + "\")' >목록</button>";
        ...
        function showCouponTable(rows,maxcount) {
            ...
            table.innerHTML = "<thead><tr class='textevent' style='background-color:#f8f9fa'><th>횟수</th><th>운동날짜</th><th>시간</th><th>상태값</th><th>설명</th><th>삭제</th></tr></thead><tfoot></tfoot><tbody></tbody>";
            table.width="100%";        
            table.cellspacing="0";
            table.border = "1";
...
//            table.className="table table-bordered";
                ...
                for (var i = 0; i < newmaxcnt; i++) {
                    ...
                    brow.style.height = "30px";
                     if(i < len){
                        ...
                        var bcell_index = brow.insertCell();
                        bcell_index.innerHTML = "<text style='font-size:12px;'>"+(i+1)+"</text>";
                        bcell_index.style.maxWidth="30px";

                        // 운동날짜       
                        var bcell_id = brow.insertCell();
                        bcell_id.innerHTML = "<text style='font-size:12px;'>"+data.date+"</text>";

                        // 운동시간 
                        var bcell_id = brow.insertCell();
                        bcell_id.innerHTML = "<text style='font-size:12px;'>"+data.hour+"시</text>";

                        // 상태값       
                        var bcell_num = brow.insertCell();
                        bcell_num.innerHTML = "<text style='font-size:12px;'>"+txt_status+"</text>";

                        // 설명
                        var bcell_pass = brow.insertCell();
                        bcell_pass.innerHTML = "<text style='font-size:12px;'>"+data.note+"</text>";
        ...
        function showCouponList(couponid,totalmaxcount,str_rows){
            var rows = JSON.parse(str_rows);
            console.log("totalmaxcount "+totalmaxcount)
            clog("rows ",rows);
            var title = "["+couponid.substr(0,10)+"] 등록횟수 : "+totalmaxcount+"회";
            clog("rows ",rows);
            var message_tag = showCouponTable(rows,totalmaxcount);
...
//        function onselect_myuser(){
//            var select_users = document.getElementById("select_users");
//            var dvalue =  getDataListSelectedValue("select_users","select_users_list");
//            var dtext =  getDataListSelectedText("select_users");
//            var uid = dvalue != "0" ? dvalue : "";
//            var username = dtext;
//            if(uid){
//                select_useruid = uid;
//                reload_calendar_page();
...
//                        clog("first userinfos ",userinfos);
                        container.style.display = "none";
                        reservation_container.style.display = "block";
                        calendar_data = json;
...
//            }
//           
////            clog("calendar_data is ",calendar_data);
...
//                alertMsg("시간 : .."+today+" "+todayhour+"  : "+checkday+" "+checkhour+" dday "+get_Day(today, checkday));//test
                ...
                //real  neel_test
//                var finishtag =  isNowTimeOver(obj[0]) <= 0 ? "<option value='2'>P.T종료</option>" : ""; //운동종료 시간이 되어야 PT종료 목록이 보인다.
                //test
                var finishtag =  "<option value='2'>P.T종료</option>"//test //20210907_neel
                
                var selecttag = "<select id='teacher_reservation_status' class='form-control' >"+
                                    "<option value=''>== 상태값을 선택하세요 ==</option>"+
                                    finishtag+
                                    "<option value='4'>P.T지우기 (횟수차감없음)</option>"+                                
                                "</select><br>"+            
                                "<label align = 'left' class='textevent'>내용입력</label><input id='id_finish_note' class='form-control' name='edittext_other' placeholder='기타 특이사항 입력란..' style='height:140px;'/>";
...
//                        return;
                    ...
                    remove_teach_user(info,obj[0],obj[1],obj[2],obj[3],parseInt(teacher_reservation_status.value),id_finish_note.value,couponid);
                }, function() {
                   hideModalDialog();
                });
            ...
            //운동완료상태 승인요청 다시하기
                ...
                var message = "<br>"+userobj.name+" 님에게 승인요청 푸시를 보내시겠습니까?<br><br>";
...
//                            "</div>";
            ...
            //운동예약신청한 상태에서 트레이너가 취소한다.
                ...
                var message = "<br>"+userobj.name+" 님 운동신청건을 취소하시겠습니까?<br><br>";

                showModalDialog(document.body, "운동예약요청 취소하기", message, "신청건 취소하기", "취소", function() {
                    var mtitle = "운동종료";
                    var mmessage = obj[0]+" 운동 예약신청이 취소되었습니다.";
                    
                    remove_teach_user(info,obj[0],obj[1],obj[2],obj[3],status,"",couponid);
                }, function() {
                   hideModalDialog();
                });
            //이부분은 오너가 삭제한다.    
            }else if(status == 2){
                var message = "<br>"+userobj.name+" 님 PT종료건을 삭제하시겠습니까?<br><br>";
...
//                    hideModalDialog();
        ...
        function remove_teach_user(info,_date ,_teachid,_user,_opentype,status,finish_note,couponid){
            var obj = info.split('|');
            var removeuser = {};
...
//            if(mergeAllData_remove(removeuser)){
            ...
            CallHandler("my_teacher_reservation", data, function(res) {
                console.log("removeteachuser ",res);
                var code = parseInt(res.code);
                is_updated = true;
                if (code == 100) {
                    var data = res.message;
...
//                    hideModalDialog();
         ...
         function insert_user(info,type){
...
//                        var newuserinfos = trim_teacher_userinfos(mem_teacher_alldata,userinfos, mid);
//                        newuserinfos.sort(sort_by('name', false, (a) => a.toUpperCase()));//이름순 정렬
                        var div = document.createElement("div");
                       
//                        var options = "<option data-value='0' value='== 회원을 선택하세요 =='/>";
                        var objs = [];
                        
                        
                        for (var i = 0; i < newuserinfos.length; i++) {
                            
                            if(newuserinfos[i].name == "강예빈")
                                clog("강예빈 ",newuserinfos[i]);
                            var iddate =  (newuserinfos[i].id).substr(0,10);//getToday(new Date(newuserinfos[i].id));
                            var maxcount = parseInt(newuserinfos[i].mbsmaxcount);
                            ...
                            var txt_str = "["+month+"/"+day+"]&nbsp;"+newuserinfos[i].name+"("+newuserinfos[i].userid+")&nbsp;"+total_maxcount+"/"+usecount+"회 이용";
                            ...
                            var rectimgname = "";
//                            teststr+= "("+i+") sday "+sday+" eday "+eday+" ";
//                            if(sday <= 0 && eday >= 0)         
//                            if(eday < 0 && isfree)
//                                txt_str = TXT_FREEPT_AND_TIMEOUT+"&nbsp;"+newuserinfos[i].name+"("+newuserinfos[i].userid+")&nbsp;"+maxcount+"/"+remaincount+"회";
//                            else if(eday < 0)    
//                               txt_str = TXT_TIMEOUT+"&nbsp;"+newuserinfos[i].name+"("+newuserinfos[i].userid+")&nbsp;"+maxcount+"/"+remaincount+"회";
//                            else if(isfree)
//                               txt_str = TXT_FREEPT+"&nbsp;"+newuserinfos[i].name+"("+newuserinfos[i].userid+")&nbsp;"+maxcount+"/"+remaincount+"회";
                            
//                            //무료PT 이라면
//                            if(type == "freept" && isfree && eday >= 0){ 
//                                txt_str = TXT_FREEPT+"&nbsp;"+newuserinfos[i].name+"("+newuserinfos[i].userid+")&nbsp;"+total_maxcount+"/"+remaincount+"회";
//                                rectimgname = "./img/rect_freept.png";
//                            }
//                            //유료PT 이면서 기간이 지났다면
//                            else if(type == "timeover" && !isfree && eday < 0){
//                                
//                                txt_str = TXT_TIMEOUT+"["+month+"/"+day+"]&nbsp;"+newuserinfos[i].name+"("+newuserinfos[i].userid+")&nbsp;"+total_maxcount+"/"+remaincount+"회";
//                                rectimgname = "./img/rect_ptoff.png";
//                            }
//                            //유료PT 이면서 기간이 안지났다면
//                            else if(type == "defaultpt" && !isfree && eday >= 0){
//                                 txt_str = "["+month+"/"+day+"]&nbsp;"+newuserinfos[i].name+"("+newuserinfos[i].userid+")&nbsp;"+total_maxcount+"/"+remaincount+"회";
//                                 rectimgname = "./img/rect_pton.png";
//                            }
//                            //오너가 삽입했다면
//                            else if(type == "ownerinsert"){
//                                if(isfree && eday >= 0){
//                                    txt_str = TXT_FREEPT+"&nbsp;"+newuserinfos[i].name+"("+newuserinfos[i].userid+")&nbsp;"+total_maxcount+"/"+remaincount+"회";
//                                    rectimgname = "./img/rect_freept.png";
//                                }
//                                else if(!isfree && eday < 0){
//                                    txt_str = TXT_TIMEOUT+"&nbsp;"+newuserinfos[i].name+"("+newuserinfos[i].userid+")&nbsp;"+total_maxcount+"/"+remaincount+"회";
//                                    rectimgname = "./img/rect_ptoff.png";
//                                }else if(!isfree && eday >= 0){
//                                    txt_str = "["+month+"/"+day+"]&nbsp;"+newuserinfos[i].name+"("+newuserinfos[i].userid+")&nbsp;"+total_maxcount+"/"+remaincount+"회";
//                                    rectimgname = "./img/rect_pton.png";
...
//                            }
                            //무료PT 이라면
                            if(type == "freept" && isfree && eday >= 0){ 
                                txt_str = newuserinfos[i].name+"("+newuserinfos[i].userid+")&nbsp;"+total_maxcount+"/"+remaincount+"회";
                                rectimgname = "./img/rect_freept.png";
                            }else if(type != "freept" && !isfree){
                                txt_str = "["+month+""+day+"]&nbsp;"+newuserinfos[i].name+"("+newuserinfos[i].userid+")&nbsp;"+total_maxcount+"/"+remaincount+"회 남음";
                                rectimgname =  eday < 0 ? "./img/rect_ptoff.png" : "./img/rect_pton.png";
                            }
                            ...
                            if(!param_useruid || param_useruid && param_useruid == newuserinfos[i].uid){//예약할때 목록을 선택한 회원만 보여줄건지 전체회원을 보여줄건지
                                // options += "<option value='"+i+"'>"+txt_str+"</option>";
                                 options += "<option data-value='"+i+"' value='"+txt_str+"' />";                                
//                                 options += "<option value='"+i+"' style='font-size:10px;'>"+newuserinfos[i].name+"("+newuserinfos[i].userid+")&nbsp;&nbsp;&nbsp;등록:"+iddate+"&nbsp;["+remaincount+"회남음]</text></option>";
                        ...
                        for(var c = 0 ; c < 12;c++){
                            ...
                            var isbr = c%4 == 3 ? "<br>" : "";
                            time_tag += "<input id='min_"+min+"' type='radio' name='ampm' value='"+min+"' "+ischecked+">&nbsp;"+min+"분&nbsp;&nbsp;"+isbr; 
                        }
                        div.innerHTML += "<text class='textevent' style='float:left;width:100%;padding:5px'>분을 선택하세요</text><div class='form-control' style='height:auto'>"+time_tag+"</div>";
                        
                        
//                        div.innerHTML += "<input id='time_select_users' type='text'  onfocus='this.select()'  onchange='onTimeSelectUser()' list='time_select_user_list' placeholder='회원을 선택하세요..' style='width:100%;height:50px' /><datalist id='time_select_user_list'>"+options+"</datalist>";                       
                        div.innerHTML += C_ComboBoxTag("time_select_users","== 회원을 선택하세요 ==",objs,0,1,"onTimeSelectUserNew",-1);
                    
                         div.innerHTML += "<div style='width:100%;height:500px'><div>"; //콤보박스가 보이지 않아서 빈공간 삽입. 임시용
                        // ""+getDateToStr(yy,mm,dd,hh)+"|"+event.teachid+"|"+myuid+"|"+event.opentype;
                        var insertstatus = type == "ownerinsert" ? 2 : 10;
                        ...
                        if(type == "freept"){
                            showModalDialog3Button(document.body, "예약하기","<br>"+date+"일 "+time+" 시 예약<br><br>"+div.innerHTML+"<br>", "예약하기", "취소", "무료PT종료",function() { //real                        
    //                        showModalDialog(document.body, "예약하기",teststr, "예약하기", "취소", function() {//test

                                var min = 0;
                                for(var i = 0 ; i < 12; i++){
                                    var m = i*5;
                                    var radio_m = document.getElementById("min_"+m);
                                    if(radio_m.checked){
                                        min = parseInt(radio_m.value);
                                        break;
                                    }
                                ...
                                var text = arg.text;
//                                var selected_user_index =  getDataListSelectedValue("time_select_users","time_select_user_list");
//                                var text =  getDataListSelectedText("time_select_users");
//                                var select_user = document.getElementById("time_select_users");
//                                var selected_user_index = select_user.value;
//                                var text = select_user.options[select_user.selectedIndex].text;
                                ...
                                //새로재정의해서 넘긴다.
                                var clickid = ""+obj[0]+"|"+obj[1]+"|"+selected_user.uid+"|"+obj[3];

                                
//                                clog("000 selected_user ",selected_user);
                                
                                insert_teach_user(insertstatus, clickid,obj[0],obj[1],selected_user.uid,obj[3],mid,mtype,centercode,selected_user.id,min,isfreept,type);//selected_user.id  = 쿠폰아이디
...
//                                    var selected_user_index = select_user.value;
                            ...
                            showModalDialog(document.body, "예약하기","<br>"+date+"일 "+time+" 시 예약<br><br>"+div.innerHTML+"<br>", "예약하기", "취소", function() { //real                        
    //                        showModalDialog(document.body, "예약하기",teststr, "예약하기", "취소", function() {//test
...
//                                var text = select_user.options[select_user.selectedIndex].text;
                                ...
                                //새로재정의해서 넘긴다.
                                var clickid = ""+obj[0]+"|"+obj[1]+"|"+selected_user.uid+"|"+obj[3];
//                                clog("222 selected_user ",selected_user);
                                insert_teach_user(insertstatus, clickid,obj[0],obj[1],selected_user.uid,obj[3],mid,mtype,centercode,selected_user.id,min,isfreept,type);//selected_user.id  = 쿠폰아이디
...
//            window.calendar(window.el,window.options);
        ...
        function finishFreePT(user_uid,couponid,teachername, teacher_uid){
            ...
            //담당트레이너 설정하기
                ...
                setPTTraner(user_uid,couponid,teachername, teacher_uid,orner_status_value,teacher_uid,teachername,param_groupcode,param_centercode,function(res){
                     var token = res.message;
                     if(res.code == 100){
...
//        function onTimeSelectUser(){
//            
////            var dvalue =  getDataListSelectedValue("time_select_users","time_select_user_list");
//            var text =  getDataListSelectedText("time_select_users");
//          
//            if(text.indexOf(TXT_FREEPT_AND_TIMEOUT) >= 0){
//                
...
//            var users = getUsers(json);
        ...
        function insert_teach_user(status,info,_date ,_teachid,_user,_opentype,managerid,mbstype,centercode,couponid,min,isfree,isownertype){
...
//                   var data = res.message;
                    ...
                    if(isownertype == "ownerinsert"){
                        alertMsg("PT운동을 추가하였습니다.",function(){
                            hideModalDialog();
                            hideModalDialog();    
                            reload_calendar_page();
                        });
...
//                C_showToast( "예약취소", date+" "+hh+"시 예약을 취소하였습니다.", function() {});
                if(status == 4) //cancel
...
//                });
        ...
        function insert_userpt(time){
            var date = new Date(time);
            var teach_datas = getTeachDatas();
            var info = time+"|||ready";
            var tag = "";
            var type = "openteachtime";
            var teachid = teach_datas[0].teachid;
            ...
            var clickid = ""+time+"|"+teachid+"||ready";
            CallHandler("my_teacher_reservation", data, function(res) {
                var code = parseInt(res.code);
                is_updated = true;
                if (code == 100) {
//                    insert_time_data(time,teachid);
//                    mergeAllData_time_update(type,timedata.date,timedata.time);    
                    insert_user(clickid,"ownerinsert");
                }
            });
        }
                ...
                <td colspan="7" style="text-align: center">
                    <table style="white-space: nowrap; width: 100%">
                        <tr>
                            <td colspan="3">
                             {{   if (mode ==='month') {  }}
                                    <button  id="total_gxroom_count" style="margin-left:10px;background-color:#333333;height:32px;color:white;font-size:12px;border-radius:16px;padding-left:15px;padding-right:15px;display:none">GX룸: 0개</button>
                    ...
                    <table class="table table-striped table-condensed table-tight-vert">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="time-{{: i}}-0">
                                    <span style="float:right;height:100%;margin-right:15px"><img src="./img/icon_add.png" width="25px' height="25px" id="{{: addid }}" onclick="insert_userpt('{{: time }}')"></span>
                                </td>
                                {{ } else { }}
                                </th>
                                <td class="time-{{: i}}-0">
...
//                    }
                ...
                var deleteicon = "<span style='float:right;height:100%;margin-top:-30px'><img src='./img/icon_delete48.png' width='25px' height='25px' onclick='remove_user_reservation(\"" + clickid + "\","+event.user.status+",\""+event.user.couponid+"\")' ></span>";
//                var addpticon = "<span style='float:right;height:100%;margin-top:-30px'><img src='./img/icon_addpt.png' width='25px' height='25px' onclick='insert_user(\"" + clickid + "\",\"freept\")' ></span>";
                if(event.user.uid){
                    var userobj = findValueinArray(userinfos,"uid",event.user.uid,event.user.couponid);
...
//                                    clog("timenote_"+clickid[0]+"_"+event.user.couponid+"");
                                    //var note = event.user.note ? "<br><text style='font-size:15px;color:blue;font-weight:bold'> - Note - "+removeFreePTText(event.user.note)+"</text>"+deleteicon : deleteicon;
                                    var note = event.user.note ? "<br><text>- Note -</text><input id='timenote_"+clickid+"' onchange='time_note_change(\"" + clickid + "\","+event.user.status+",\""+event.user.couponid+"\",\""+isfreepttxt+"\")' value='"+removeFreePTText(event.user.note)+"'/>"+deleteicon : deleteicon;
                                    text = statustxt+"<text style='color:blue'>"+userobj.name+"["+userobj.userid+"]</text>"+note;
    //                                if(mm == 10 && dd == 19)clog("00 userobj is ",userobj);
                                }                                
                                else {
//                                    var note = event.user.note ? "<text style='font-size:15px;color:blue;font-weight:bold'> - 시간 : "+removeFreePTText(event.user.note)+"&nbsp;-</text><br>"+deleteicon : deleteicon;
                                    var note = event.user.note ? "<text> 시간 : </text><input id='timenote_"+clickid+"' onchange='time_note_change(\"" + clickid + "\","+event.user.status+",\""+event.user.couponid+"\",\""+isfreepttxt+"\")' value='"+removeFreePTText(event.user.note)+"'/><br>"+deleteicon : deleteicon;
                                    ...
                                    var maxcount = parseInt(userobj.mbsmaxcount);
                                    var freecount = userobj.mbsfreecount ? parseInt(userobj.mbsfreecount) : 0;

    //                                if(mm == 10 && dd == 19)clog("11 userobj is ",userobj);
                                    text = note+statustxt+"<text style='color:white'>"+userobj.name+"["+userobj.userid+"], "+userobj.mbsdesc+" "+TXT_WON+userobj.mbsprice+" <br>"+maxcount+"회중 "+userobj.usecount+"회사용, 기간 : "+stime+" ~ "+etime+" ("+userobj.mbsmonth+"개월)"+"</text>"; 
                                }
                            }else {
...
//                            if(mm == 10 && dd == 19)clog("22 userobj is ",userobj);
                            text = statustxt+userobj.name+"["+userobj.userid+"]";    
                        }
                        
                        
                        if(event.user.note.indexOf(TXT_FREEPT) >= 0){
                            text ="<img src = './img/icon_freept.png' style='margin-top:-30px;margin-left:-50px;position:absolute;' title='무료PT일때 이 아이콘이 표시됩니다.'/>"+text;
                        }
...
//                if(mm == 10 && dd == 8){
////                    clog(yy+"-"+mm+"-"+dd+" text is "+text);
//                    clog("text is ",text);
//                   // text = "<text style='color:blue;font-weight:bold'>[운동완료]&nbsp;</text><text style='color:white'>임다은[100724)</text><br><text style='font-size:15px;color:blue;font-weight:bold'> - Note - 19시 0분</text>";
//                }
...
//                
                ...
                if( event.user.uid){
                    ...
                    {
                         div.innerHTML = "<button class='btn btn-primary' style='background-color:#aa00aa' onclick='insert_user(\"" + clickid + "\",\"freept\")' >무료PT</button>&nbsp;&nbsp;<button class='btn btn-primary' style='background-color:#0000af' onclick='insert_user(\"" + clickid + "\",\"defaultpt\")' >유료PT예약</button>&nbsp;&nbsp;<span style='float:right'><img onclick='remove_time(\"" + clickid + "\")' type= 'button' src ='./img/btn_close_50.png' style='margin-top:-4px'/></span>&nbsp;&nbsp;";
                    }  
                }
...
//                        var btn_txt = event.user.status == 0 ? "운동완료" : "승인요청 다시하기";
                        div.innerHTML = isdisplayuser ? div.innerHTML+"<button id='btn_"+clickid+"' class='btn btn-primary' style='float:right;margin-top:-9px;"+color_tag+"' onclick='remove_user_reservation(\"" + clickid + "\","+event.user.status+",\""+event.user.couponid+"\")' >"+btn_txt+"</button>" : div.innerHTML;
                    }  
                }
...
//                    if(event.fontcolor)$event.css("color",event.fontcolor);
                ...
                if(event.fontcolor)div.style.color = event.fontcolor;
                 ...
                 //수동입력이라면
                if(hour < 6){
                    div.style.backgroundImage = "linear-gradient(to bottom, #e1ece9 0px, #f1fcf9 100%)";
                    div.style.color = "black";
//                    clog("div.innerHTML ",div.innerHTML);
                }
                //////////////////////////////////////////////////////////
                //neel change end
                //////////////////////////////////////////////////////////
...
//                    if(event.user.uid == "oU0Y4voOR6N1ZvP6cVKB_2021-09-01"){
//                        clog("================================",sdate);
//                        
//                    }
...
//                    
                    if(userobj){
//                        if(userobj.name == "서문희라")clog("event ",event);
                        ...
                        text = hour < 6 && event.user.uid ? userobj.name+"["+cid+"]" : "고객번호("+userobj.userid+")";    
                    }
...
//                    if(event.timefontcolor)
//                        //$event.html(free_tag+ '<strong style="color:'+ event.timefontcolor +'">' + time + '</strong> ' + $event.html());
...
//                        $event.html(free_tag+ timetag + $event.html());
                    ...
                    $event.html(free_tag+timetag +name_tag+cid_tag+ $event.html());
                }
                if (!event.end) {
...
//                        
                        ...
                        //수동입력이라면
                        if(hour < 6)
                            $event.css("background-image","linear-gradient(to bottom, #e1ece9 0px, #f1fcf9 100%)");
            ...
            function showGXRoomCount(options){
                ...
                if(mode == "month"){
                    ...
                    if(totalgxroomcount[ym]){
                        total_gxroom_count.innerHTML = "GX방&nbsp;&nbsp;"+totalgxroomcount[ym]+"개&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;예약&nbsp;&nbsp;"+userlen+"건";
                        total_gxroom_count.title = "GX방 개수 "+totalgxroomcount[ym]+"개 , 총 예약인원 : "+userlen+"명";
                    }
...
//                var isover = get_Day(today,now);//날짜가 지났는지 체크
                ...
                var isover = compare_date(start, getNowDateTime()); //현재시간기준으로 표시함
...
//                    
        ...
        //            end = !j ? null : new Date(y, m, d + j, hh + 2, mm);
                    ...
                    if(parseInt(datas[i].times[j].members[k].status) == 0){
                         backimg = "linear-gradient(to bottom, #cfa0b8 0px, #9f8098 100%)";
                        fontcolor = "#333333";
                    }
//                    if(datas[i].times[j].members[k].status && parseInt(datas[i].times[j].members[k].status) == 0){
//                        
...
//                        
                    ...
                    var b_maxcount = 0;
                    var b_nowptcount = 0;
                    var b_datas =   {"date":b_date,"uid":b_uid,"status":b_status,"couponid":b_couponid,"maxcount":b_maxcount,"nowptcount":b_nowptcount};
//                    
                    
                    //test
//                    if(datas[i].times[j].members[k].couponid.indexOf("19:31:44") >= 0){
//                        clog(testcnt+" : "+datas[i].date+" "+datas[i].times[j].members[k].note+" status : "+datas[i].times[j].members[k].status);
//                        testcnt++;
//                    }
...
//            clog("user "+user.name+" isfree "+isfree+" mbsname " +user.mbsname);
...
//        displayUserCouponTable(selected_uid);
    ...
    function reload_calendar_page(centercode){
        ...
        param_day = mode == "day" ? date : "";
        refresh_page("?groupcode="+param_groupcode+"&centercode="+param_centercode+"&teacheruid="+param_teacheruid+"&teacherid="+param_teacherid+"&teachername="+param_teachername+"&useruid="+select_useruid); 
