...
<!--<text id="title_debug" style="float:left;margin-left:30px;font-size: 12px;color:#181c32;text-align:left;font-weight:400;" ></text><text id="title_debug2" style="float:left;margin-left:30px;font-size: 12px;color:#181c32;text-align:left;font-weight:400;" ></text>-->
...
<div id="alltip" style="position:absolute;display:none" ></div>  
  ...
  if (mode === 'week') {
    ...
    <div style="width: 1260px; background-color: #fff; border-radius: 10px; border-bottom-left-radius: 0px; border-bottom-right-radius: 0px; z-index: 1000;box-shadow: 0px 0px 5px 0px rgb(0 0 0 / 5%);border: 1px solid #eff2f5; margin-top:-48px; margin-left: -17px; margin-right:-17px; position: fixed;">
        <div id="div_gxtitle" style="width:100%;height:auto; padding-top:25px; padding-right:10px;">
            <!-- 타이틀-->
        <text id="title_day" style="float:left;margin-left:30px;font-size: 18px;color:#181c32;text-align:left;font-weight:700;" >{{: user_name}} GX 현황</text>
        ...
        <div id="id_hcalendar" style="width:100%;">
            <div style="height:50px">
                <span style="float:left;margin-top:5px">
                   ...
                   <svg style="width:8px;height:8px"><rect width="8" height="8" rx="4" ry="4" style="fill:#f2329a;"></rect></svg>&nbsp;<text id="txt_total_gxuser_month" style="font-size:14px; color:#3f4254;font-weight:500;">총 예약인원: 0명</text>&nbsp;&nbsp;
        ...
        <div id="div_cbuttons" align="center" style="width:100%;height:80px;margin-top:15px; padding-left:10px; padding-right:10px;">
                    ...
                    <div align="left" style="margin-top:-50px;">
                        ...
                        <svg style="width:8px;height:8px"><rect width="8" height="8" rx="4" ry="4" style="fill:#f2329a;"></rect></svg>&nbsp;<text id="txt_total_gxuser_day" style="font-size:14px; color:#3f4254;font-weight:500;">총 예약인원: 0명</text>&nbsp;&nbsp;
                    <svg style="width:8px;height:8px"><rect width="8" height="8" rx="4" ry="4" style="fill:#7768d8;"></rect></svg>&nbsp;<text id="txt_total_gxreadyuser_day" style="font-size:14px; color:#3f4254;font-weight:500;">총 대기인원: 0명</text>&nbsp;&nbsp;
                   ...
                   <!--<button class="js-cal-option btn btn-default {{: first.toDateInt() <= today.toDateInt() && today.toDateInt() <= last.toDateInt() ? 'active':'' }} " style="float:left;margin-top:8px;background-color:#3e7db8;height:38px;color:white;font-size:13px;margin-left:10px;" onclick="loadMainDiv(34)">Today</button>
                    ...
                    <button id="btn_insert_room" style="visibility:hidden;float:right;margin-top:8px;margin-right:10px;background-color:#3e7db8;height:38px;color:white;border:0px;border 0px;border-radius:3px;font-size:13px;padding-left:15px;padding-right:15px" onclick="insert_gxroom();updateSetTutorialStatus();">강좌입력하기</button>
    ...
    <div id="div_gxtitle" style="width:100%;height:auto">
        <!-- 타이틀-->
       <text id="title_month" style="float:left;margin-left:10px;font-size: 18px;color:#181c32;text-align:left;font-weight:700;" >{{: user_name}} GX 현황</text>
       <!-- 회원검색-->
    ...
    <!--</div>-->
    {{ if (mode == 'day') { }}
    <table id="gx_calendar" style="width:100%; margin-top:163px;" >
    {{ }else{ }}
    <table id="gx_calendar" style="width:100%"  >
    {{ } }}
      ...
      month = 0;
        ...
        <tbody>
            ...
            <tr style="height:140px">
                ...
                <td id="monthtd_{{: now }}" class="calendar-day {{: dayclass }} {{: thedate.toDateCssClass() }} {{: date.toDateCssClass() === thedate.toDateCssClass() ? 'selected':'' }} {{: daycss[i] }} js-cal-option  stop-dragging" data-date="{{: thedate.toISOString() }}" style="border:1px solid #eaf0f1;vertical-align: top;" onmouseover="td_mouseover('{{: now }}')">
...
<script>
    ...
    //마우스 오버된 날짜가 빈날짜이면 말풍선을 사라지게 한다.
    function td_mouseover(date){
        
//        console.log("td_mouseover ",date);
//        console.log("date "+date+" roomindates "+roomindates[""+date]);
        if(date && !roomindates[""+date]){
            $("#alltip").hide();
            alltip_hour_index = 0;
        }
    ...
    function gxPermissionCheck(){
        ...
        btn_insert_room.style.visibility = window.options.mode == "day" && isPermission(222) ? "visible" : "hidden";
        update_minicalendar();
    }
...
//    if(!isPermission(227))btn_download_excel.style.visibility = "hidden";
...
//        clog("onup_hdate "+ismousedown_hdate);
    ...
    function onClickRoomCopy(){
        ...
        if(isinPasteRooms(alldata, before_pastedates)){
            showModalDialog3Button(document.body,  "강좌그룹 복사하기", "복사하기할 날짜에 기존강좌가 있습니다. <br>기존강좌들을 삭제하고 삽입할까요?", "삭제하고 삽입하기", "삭제하지 않고 삽입하기", "취소", function(){
                
                showModalDialog(document.body, "경고", "붙여넣기할 날짜 기존강좌들이 모두 삭제됩니다.", "삭제하고 삽입하기", "취소", function() {
                
                   pasteRooms(before_copydates,before_pastedates,input_minicalendar_opentime.value,true);
                }, function() {
                    hideModalDialog();
                });
                ...
                pasteRooms(before_copydates,before_pastedates,input_minicalendar_opentime.value,false);    
            }, function(){
                hideModalDialog();
            }); 
        }else {
    ...
    function isinPasteRooms(alldata, pastedates){
        var isin = false;
       
        for(var i = 0; i < pastedates.length;i++){
            for(var j = 0; j < alldata.length;j++){
                var date = new Date(alldata[j].start);
                var yy = date.getFullYear();
                var mm = parseInt(date.getMonth()+1) < 10 ? "0"+(date.getMonth()+1) : ""+(date.getMonth()+1);
                var dd = parseInt(date.getDate()) < 10 ? "0"+(date.getDate()) : ""+date.getDate();
                var yymmdd = yy+"-"+mm+"-"+dd;
                if(yymmdd == pastedates[i]){
                    isin = true;
                    break;
                }
            }
...
//        여기부터해야함
       ...
       // var message = copy_multi_date[0]+" ~ "+copy_multi_date[copy_multi_date.length-1]+" 강좌정보 데이타를 "+click_hdate_arr[0]+" ~ "+click_hdate_arr[click_hdate_arr.length-1]+" 날짜로 붙여넣기 합니다.<br>"+
        ...
        showModalDialog(document.body, "강좌정보 복사", message, "강좌정보 복사하기", "취소", function() {
            ...
            pasteRooms(copy_multi_date,click_hdate_arr,id_paste_opentime.value,false);
...
//               div_main.style.display = "block";   
               ...
               C_showToast( "에러", res.message, function() {});
...
//        var keys = ["select_gxtype","select_gxmin","select_gxteacher","input_gx_max","input_opentime","input_reservationmin","input_cancel_reservationmin","select_isshow","input_gx_note"];
        ...
        switch(key){
            case 1://select_gxtype
                ...
                var insert_date = arg1;//ex)arg1 =  date : 2023-05-04
                var thisweek_maxreservation = getThisWeekMaxReservation(gxtype.gxmaxreservation,insert_date);
//                console.log("arg1 ",arg1); 
                ...
                if(input_maxreservation.value == "0")input_maxreservation.value = thisweek_maxreservation;
    ...
    function getThisWeekMaxReservation(gxmaxreservation,date){
        console.log("alldata, ",alldata);
        //해당날짜 주
        var weeks = get7Days(date);
//         console.log("weeks, ",weeks);
        var isweekroommax = parseInt(gxmaxreservation);
        for(var j = 0 ; j < weeks.length;j++){
            for(var i = 0 ; i < alldata.length;i++){
                if(alldata[i].roomdata.room_datetime.indexOf(weeks[j]) >= 0){
                    isweekroommax = parseInt(alldata[i].roomdata.room_maxreservation);
                    break;
                }
            }
        }
        return isweekroommax;
    }
...
//            var rmaxreservation = gxtype && gxtype.gxmaxreservation ? parseInt(gxtype.gxmaxreservation) : 0; // GX 최대예약횟수는 모든 GX 통합이다.
            var rmaxreservation = roomdata  ? parseInt(roomdata.room_maxreservation) : parseInt(gxtype.gxmaxreservation);  // GX 최대예약횟수는 모든 GX 통합이다.
...
//             clog("111 gxtype ",gxtype);
        ...
        var gxtype_tag = "<select onchange='onchange_gxroomdata(1,\""+minopentime+"\",\""+rdatetime+"\")'id='"+keys[0]+"' style='width:95%;height:45px;font-size:14px;background-color:#f5f8fa;border-radius:10px;padding-left:15px;border:0px'>"+gxtype_optiontag+"</select>";            
...
//        var gxautoreadyin_tag = "<select onchange='onchange_gxroomdata(12,\""+minopentime+"\")' id='"+keys[11]+"'  style='width:100%;height:45px;font-size:14px;background-color:#f5f8fa;border-radius:10px;padding-left:15px;border:0px'>"+gxautoreadyin_optiontag+"</select>";         
        ...
        ////////////////////////////////////
        //GX maxreservation 세팅  주당예약가능 횟수
        ////////////////////////////////////
         
        
        var gx_maxreservation_tag = roomid ? "<input onclick='onclick_week_maxreservation("+rmaxreservation+",\""+rdatetime+"\", "+isroom_insert+")' id='"+keys[12]+"' placeholder='1주에 최대 예약가능한 횟수를 입력하세요...' value='"+rmaxreservation+"'  style='width:100%;height:45px;font-size:14px;background-color:#f5f8fa;border-radius:10px;padding-left:15px;border:0px' readonly/>" : "<input id='"+keys[12]+"' placeholder='1주에 최대 예약가능한 횟수를 입력하세요...' value='"+rmaxreservation+"'  style='width:100%;height:45px;font-size:14px;background-color:#f5f8fa;border-radius:10px;padding-left:15px;border:0px' />";;
        ...
        //GX 예약하기 최소시간 
        ////////////////////////////////////
        var gx_reservationtime_tag = "<input onchange='onchange_gxroomdata(5,\""+minopentime+"\")'  type='number' id='"+keys[5]+"' placeholder='최소 예약가능시간(분)을 입력하세요...' value='"+rreservationmin+"' style='width:95%;height:45px;font-size:14px;background-color:#f5f8fa;border-radius:10px;padding-left:15px;border:0px'/>";
        
        ////////////////////////////////////
        //GX 예약취소 최소시간
        ////////////////////////////////////
        var gx_cancelreservation_tag = "<input onchange='onchange_gxroomdata(6,\""+minopentime+"\")' type='number' id='"+keys[6]+"' placeholder='최소 예약취소시간(분)입력하세요...' value='"+rcancelreservationmin+"' style='width:95%;height:45px;font-size:14px;background-color:#f5f8fa;border-radius:10px;padding-left:15px;border:0px'/>";
                                ...
                                "<table style='width:auto;height:auto;' >"+
                                    "<tbody style='width:100%;float:center;'>"+
                                        ...
                                        "<tr style='width:100%;height:55px'>"+
                                         ...
                                         "<tr style='width:100%;height:55px'>"+                                        
                                            "<td>"+
                                                "<label id='label_"+keys[5]+"' style='font-size:15px; color:#181c32;text-align:left; font-weight:500;margin-top:20px'>예약하기 최소시간(분)</label>"+
                                            "</td>"+
                                            "<td>"+
                                                "<label id='label_"+keys[6]+"' style='font-size:15px; color:#181c32;text-align:left; font-weight:500;margin-top:20px'>예약취소 최소시간(분)</label><br>"+
                                            "</td>"+
                                            "<td>"+
...
//            clog("000 createrooms");
    ...
    function onclick_week_maxreservation(nowmax,nowdatetime,isroom_insert){
//        console.log("nowmax "+nowmax+" nowdatetime "+nowdatetime);
//        console.log("days ",get7Days(nowdatetime));
        var week_days = get7Days(nowdatetime);
        var date = nowdatetime.substr(0,10);
        var str_days = "";
        for(var i = 0 ; i  < week_days.length;i++){
            str_days += i == 0 ? week_days[i] : ", "+week_days[i];
        }
       
       showModalDialog(document.body, "경고!", "<br><label style='font-size:14px;'>선택한 주<br><span style='font-weight:bold;color:#ff5555'>[ "+str_days+" ]</span><br>방들의 예약가능횟수가 모두 바뀝니다.</label><br><br>최대 예약가능횟수 : <input type='number' id='input_week_maxreservation' value='"+nowmax+"' style='width:100px;height:45px;font-size:16px;background-color:#f5f8fa;border-radius:10px;padding-left:15px;border:0px'> 회<br>", "설정하기", "취소", function() {
           var input_week_maxreservation = document.getElementById("input_week_maxreservation");
             changeWeekMaxReservation(week_days,input_week_maxreservation.value);
         },function(){
             hideModalDialog();
         });
        
    }
    function changeWeekMaxReservation(week_days,week_maxreservation){
       
        var groupcode = getData("nowgroupcode");
        var centercode = getData("nowcentercode");
        var data = {
            type: "updategxweekreservation",
            groupcode : groupcode,
            centercode : centercode,                
            value: {"days" : week_days,"weekmaxreservation":parseInt(week_maxreservation)}
        };
...
//            }) 
    ...
    //isallclean 붙여넣기 할 날짜 기존강좌 삭제하기 여부
    function pasteRooms(copydates,pastedates,opentime,_isallclean){
        
        var groupcode = getData("nowgroupcode");
        var centercode = getData("nowcentercode");
        var pasteholidays = getHolidays(pastedates);
        var isallclean = _isallclean ? "1" : "0";
        var data = {
            type: "pastegxrooms",
            groupcode : groupcode,
            centercode : centercode,                
            value: {"copydates" : copydates ,"pastedates":pastedates,"pasteholidays":pasteholidays,"opentime":opentime,"isallclean":isallclean}
        };
...
//            }) 
    ...
    function init_d_settinggx(value){
...
//            thisday = date_arr[2];
            ...
            user_name_id = value.username;
            
            if(user_name_id){
                user_name = user_name_id.split(" ")[0];
            }
        ...
        showCenterGX(getData("nowcentercode"),thisyear);
...
//        var title_debug2 = document.getElementById("title_debug2");
        document.addEventListener("mousemove", (e) => { // mousemove이벤트를 이용해 움
            ...
            // pageX, pageY와는 다름.
//            console.log("alltip.style.height ",alltip.clientHeight);
            const mouseX = e.clientX/(screen_zoom/100);
            const mouseY = e.clientY/(screen_zoom/100);
//            alltip.style.left = (mouseX + scrollPositionX+60)+'px';
//            alltip.style.top = (mouseY+scrollPositionY-alltip.clientHeight-90) + 'px';
//            console.log("alltip_week "+alltip_week);
            var startx = alltip_week == "목" || alltip_week == "금" || alltip_week == "토" ? -600 : 15;
            var starty = 200;
           
            var content_height = alltip.clientHeight+70;
            alltip.style.left = (mouseX+startx)+'px';
            alltip.style.top = (mouseY+scrollPositionY/(screen_zoom/100)-content_height) +'px';
//            title_debug2.innerHTML = " mouseX "+mouseX+" mouseY "+mouseY+" scrollPositionY "+scrollPositionY;

//            console.log("alltip.style clientHeight "+alltip.clientHeight);
//            console.log("alltip.style scrollHeight "+alltip.scrollHeight);
//            console.log("alltip.style offsetHeight "+alltip.offsetHeight);
            
//            console.log("left "+(mouseX + scrollPositionX));
//            console.log("top "+(mouseY + scrollPositionY));
            
            var pos = getElementPos(document.getElementById('gx_calendar'));
//            console.log("scrollPositionX is "+scrollPositionX+" scrollPositionY "+scrollPositionY);
//            console.log("mouseX is "+mouseX+" mouseY "+mouseY);
            
            if(!isTableArea(pos,scrollPositionX,scrollPositionY,mouseX,mouseY)){
                $("#alltip").hide();
                alltip_hour_index = 0;
            }
    ...
    function isTableArea(table_pos,scrollPositionX,scrollPositionY,mouseX,mouseY){
        var flg = false;
        var x1 = table_pos.x+scrollPositionX;
        var y1 = table_pos.y-scrollPositionY;
        var x2 = x1+table_pos.w;
        var y2 = y1+table_pos.h;
       
        var mX = mouseX;
        var mY = mouseY;
       // var mX = scrollPositionX+mouseX;
    //    var mY = scrollPositionY+mouseY;
        
        if(mX > x1 && mX < x2 && mY > y1 && mY < y2){
//            console.log(table_pos.y+" IN mX "+mX+" mY "+mY+" :: x1 "+x1+" x2 "+x2+" y1 "+y1+" y2 "+y2);
            flg = true;
        }            
        else {
//            console.log(table_pos.y+" OUT mX "+mX+" mY "+mY+" :: x1 "+x1+" x2 "+x2+" y1 "+y1+" y2 "+y2);
            flg = false;
        }
...
(function ($) {
  ...
  function calendar($el, options) {
    ...
    $el.on('click', '.js-cal-prev', function () {
         clog("prev click 000");
//        console.log("<- 00 options.date.getMonth() "+options.date.getMonth());
         switch(options.mode) {
              case 'year': 
...
//         console.log("<- 11 options.date.getMonth() ",options.date);
        
        checkYearData(options.date.getFullYear());
//        console.log("<- 22 options.date.getMonth() "+options.date.getMonth());
        param_day = options.mode == "day" ? options.date.getTime() : "";
//        console.log("<- 33 options.date.getMonth() "+options.date.getMonth());
        setYearMonth(options.date,options.mode);
//        console.log("<- 44 options.date.getMonth() "+options.date.getMonth());
        draw();
        holidayCheckNoText();
...
//          console.log("-> 00 options.date.getMonth() ",options.date);
          switch(options.mode) {
            case 'year':
                  options.date.setFullYear(options.date.getFullYear() + 1); 
                  break;
            case 'month': 
//                    console.log("-> aa options.date.getMonth() "+options.date.getMonth());
                  options.date.setDate(1);options.date.setMonth(options.date.getMonth() + 1); 
//                    console.log("-> bb options.date.getMonth() "+options.date.getMonth());
                  break;
            case 'week': 
...
//        console.log("-> 11 options.date.getMonth() "+options.date.getMonth());
        checkYearData(options.date.getFullYear());
//        console.log("-> 22 options.date.getMonth() "+options.date.getMonth());
        param_day = options.mode == "day" ? options.date.getTime() : "";
//  console.log("-> 33 options.date.getMonth() "+options.date.getMonth());
...
//        console.log("dayadde index"+index+" max "+max);
...
//                        console.log(uinfo.name+" "+uinfo_starttime.couponid+" use "+uinfo.usecount+" 예약횟수 "+getReservationUserCouponCount(uinfo.uid+"_"+uinfo.couponid));
                        ...
                        var tag = "<div style='"+border_top_tag+"border-bottom:1px solid #ebedf3;margin-right:10px;height:100%;background-color:"+bgcolor+"'>"+
                                    "<span style='float:left'>"+
                                        "<label  onclick='showUserInfoPopup(\"" +uinfo.userid + "\")'  style='font-size:14px;font-weight:400;margin-top:10px;margin-left:10px;cursor:pointer' title='"+desc+"'>"+(num+1)+".&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+uinfo.name+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+replaceStr(uinfo.couponname,9)+" (<span style='color:#2194f3'>"+user_usercount+"</span>/"+uinfo.maxcount+")</label>"+
                                    "</span>"+
                                    "<span style='float:right;'>"+
            ...
            var title_color = users.length >= roommax ? "#fa637422" : "#e5f3da";
...
//            div.innerHTML ="<table style='width:100%;"+roombg+"'><tr><td onclick='roomclick( \""+dateFormat(event.start)+"\", "+yearmonthnum+" , \""+roomid+"\"  )' style='float:left;width:27%;font-size:15px;padding:10px;height:100%;'>"+roomtext+"</td><td style='float:left;width:63%;height:100%;font-size:15px;padding:10px;background-image:linear-gradient(to bottom, #ffe0b8 0px, #f8e58c 100%);'>"+userstext+insertuser_tag+"</td><td >"+getIsShowToggleTag(room.room_isshow,dateFormat(event.start),yearmonthnum,roomid,setJSONStringToParamString(room))+"<image onclick='removeRoom(\"" +yearmonthnum + "\",\"" +roomid + "\",\"" +hour + "\",\"" +roomname + "\",\"" +userslen + "\")'  src='./img/btn_close_50.png' style='float:right;margin-top:-10px' title='선택한 강좌를 삭제합니다.'/></td></tr></table>";
            ...
            if(dateclass == sdate ){
                var trid = "daytime_"+getYMDH(yy,mm,dd,hh+"");
        ...
        //맨마지막까지 모두 그렸다면 월달력에서 선택한 위치로 이동한다.
        if(index == max)
            scrollToRoom("daytime_"+getYMDH(yy,mm,dd,hh+""))
        
       
        
    }
    function scrollToRoom(trid){
//        var trid = "daytime_"+getYMDH(yy,mm,dd,hh+"");
        var time_tr = document.getElementById(trid);

        var tr_heights = [];
//        console.log("alltip_hour_index is "+alltip_hour_index);
        for(var i = 0 ; i < time_tr.parentElement.children.length;i++){
            var node = time_tr.parentElement.children[i];
            if(node.id.indexOf("daytime_") >= 0 && node.style.display == "block"){
                tr_heights.push(node.scrollHeight);
            }
        }
        var sheight = 0;
        for(var i = 0 ; i < tr_heights.length; i++){
            if(i < alltip_hour_index){
                sheight += tr_heights[i];
            }
        }
//        console.log("alltip_hour_index "+alltip_hour_index+" sheight "+sheight);
        $('html, body').animate({scrollTop: sheight+"px"},400);
    }
...
//        var roomid = event.roomdata.room_id;
        ...
        if(dateclass == sdate)
        {
            ...
            if(!monthdaytime_indexs[sdate])monthdaytime_indexs[sdate] = [];
            monthdaytime_indexs[sdate].push({"hour":hour,"index":index});
            var users = room.room_users ? room.room_users : [];
            var ready_users = room.room_readyusers ? room.room_readyusers : [];
...
//                console.log("show "+sdate);
                 ...
                 alltip_week = getDayOfWeek(event.start);
                
                 $("#alltip").html(roomTipTag(index,event,room));
...
//            $('.' + event.start.toDateCssClass()).append(label_maxuser);
    ...
    // 선택한 인덱스값을 세팅
    function setAlltipIndex(sdate,index_num){

        //하나의 tr = 1시간 묶음 , 하나의 tr 안에 여러개 방이 있을 수 있음 
        var realindex = 0;
        var before_hour = 0;
        if(monthdaytime_indexs[sdate])
        for(var i = 0 ; i < monthdaytime_indexs[sdate].length; i++){
            if(i == 0){
                before_hour = monthdaytime_indexs[sdate][i].hour;
            }
            else if(before_hour != 0 && before_hour != monthdaytime_indexs[sdate][i].hour){               
                before_hour = monthdaytime_indexs[sdate][i].hour;
                realindex++; 
            }
            
            
            if(monthdaytime_indexs[sdate][i].index == index_num){
                break;
            }
        }
        alltip_hour_index = realindex;
        
    }
    function roomTipTag(index,event,room){
         var roomname = room.room_name;
        var roomdetailname = room.room_detailname ? room.room_detailname : "";
        var roommax = parseInt(room.room_max);
        var users = room.room_users ? room.room_users : [];
...
//                console.log("uid_couponid "+uid_couponid);
//                console.log(":allusers[uid_couponid] ",allusers[uid_couponid]);
...
//                        console.log("desc = ",desc);
                        ...
                        var tag = "<div style='"+border_top_tag+"border-bottom:1px solid #ebedf3;margin-right:10px;height:100%;background-color:"+bgcolor+"'>"+
                                    ...
                                    "</span>"+
                                    "<span style='float:right'>"+
										"<label style='font-size:12px;font-weight:400;margin-top:10px;margin-right:10px'>(<span style='color:#2194f3'>"+user_usercount+"</span>/"+uinfo.maxcount+")</label>"+
							        "</span>"+
            ...
            var title_color = users.length >= roommax ? "#feeaec" : "#e5f3da";
            if(parseInt(room.room_isshow) == 0 || !isopentime_open)title_color = "#eeeeee";
            div.innerHTML = //타이틀 DIV
...
//        div_member_table.innerHTML = "<table class='table table-bordered' id='dataTable' width='100%' cellspacing='0'></table>";
        ...
        table.className="table table-bordered";
        table.innerHTML = "<thead><tr style='background-color:#f8f9fa'><th>순번</th><th>이름</th><th>고객번호</th><th>회원권</th><th>사용횟수</th><th>버튼</th></tr></thead><tfoot></tfoot><tbody></tbody>";
        var head = table.getElementsByTagName("thead")[0];
        var body = table.getElementsByTagName("tbody")[0];
        var foot = table.getElementsByTagName("tfoot")[0];
        var len = rows.length;
        ...
        if (len > 0) {
            for (var i = 0; i < len; i++) {
                ...
                bcell_name.innerHTML = "<button class='btn btn-primary btn-raised' onclick='showUserInfoPopup(\""+row.userid+"\")' style='background-color:#116666' >"+row.username+"</button>";
...
function insertCalenderDatas(datas){   
    ...
    for(i = 0; i < datas.length; i++) {
        if(!datas[i].gx_roomdata)continue;
                ...
                for(var k = 0 ; k < roomusers.length; k++){
                    ...
                    if(coupon){
                        if(!allusers[uid_couponid]){
                            ...
                            allusers[uid_couponid].maxcount = getMbsMaxCount(coupon);
...
//    console.log("reservation_user_couponcount ",reservation_user_couponcount);
        alldata = alldata.sort(function(a, b) {return (+a.start) - (+b.start);});
        if(isfirstinsert){
           isfirstinsert = false;
        ...
        roomindates = trim_sort_1array(roomindates);
//        console.log("roomindates ",roomindates);
    
//         var gx_calendar = document.getElementById("gx_calendar");
//            gx_calendar.onmouseout = function(){
//                console.log("gx_calendar out ");
//                
//                $("#alltip").hide();
//                alltip_hour_index = 0;
//                
//            }
    ...
    function initTotalGXUser(mode, y,m,d){
        ...
        //월별
            ...
            txt_total_gxuser_month.innerHTML = total_gxuser_month[ym] ? "총예약: "+total_gxuser_month[ym]+"건" : "총예약: 0건" ;
            txt_total_gxreadyuser_month.innerHTML = total_gxreadyuser_month[ym] ? "총대기: "+total_gxreadyuser_month[ym]+"건" : "총대기: 0건";    
        }
    }
        ...
        if(thisday && !first_goto_day && thisday > 0){
            var fInterval = setInterval(function(){
                if(isloaddata && window.options && window.mdraw){
                    ...
                    btn_insert_room.style.visibility = isPermission(222) && options.mode == "day" ? "visible" : "hidden";
                    first_goto_day = true;
                }
            },100);
        }
