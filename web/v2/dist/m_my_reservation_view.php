...
-->
   ...
   <!--    당겨서 새로고침-->
    ...
    <script src="js/scripts.js?ver3.02rrb3"></script>
...
//        var reservationinfo = null;
       ...
       document.title = decodeURI(param_username)+" 예약화면";
        //당겨서 새로고침
        PullToRefresh.init({
            mainElement: '#main',
            onRefresh: function() { 
...
//                    }
                    ...
                    var uid_couponid = param_uid+"_"+json.id;
                    var user_usecount = getGXCouponUseCount(uid_couponid,usecount); //usecount-getGXCouponReservationCount(uid_couponid);
                    var subtitle_tag = "<div style='margin-top:16px;width:100%;height:30px'>"+
        ...
        var selectgxcoupon = null;
            if(allgxcoupons.length > 0){
                ...
                for(var i = 0 ; i < allgxcoupons.length; i++){
                    ...
                    console.log(i+" gxcoupon ",selectgxcoupon);
                    console.log(i+" start "+starttime+" endtime "+endtime);
                    ...
                    if(isNowTimeMinOver(starttime) <= 0 && isNowTimeMinOver(endtime) >= 0 ){
                        ...
                        console.log("00 selectgxcoupon ",selectgxcoupon);
                        break;   
                    }
                ...
                for(var i = 0 ; i < allgxcoupons.length; i++){
                    ...
                    if(isNowTimeMinOver(starttime) > 0 && isNowTimeMinOver(endtime) > 0){
                        selectgxcoupon = allgxcoupons[i];
                        console.log("11 selectgxcoupon ",selectgxcoupon);
                        break;   
                    }              
                }
                if(selectgxcoupon == null)
                for(var i = 0 ; i < allgxcoupons.length; i++){
                    ...
                    if(isNowTimeMinOver(starttime) < 0 && isNowTimeMinOver(endtime) < 0){
                        selectgxcoupon = allgxcoupons[i];
                        console.log("22 selectgxcoupon ",selectgxcoupon);
                        break;   
                    }              
                }
                //최종적으로 첫번째 쿠폰을 등록한다.
...
//        }
        function update_reservation_user(nextstatus,info,_date ,_teachid,_user,_opentype,managerid,mbstype,centercode,couponid,note){
            alertMsg("이곳에서는 회원 예약화면을 볼수만 있습니다.");
            //neel_break
//            var obj = info.split('|');
//            var insertuser = {};
...
//                              console.log("111 options.date.getMonth() ",options.date.getMonth());
                            break;
                        case 'week':
                        ...
                        $this.on('click', '.js-cal-prev', function() {
                            ...
                            switch (options.mode) {
                                case 'year':
                                    ...
                                    break;
                                case 'month':
//                                    console.log("000 options.date.getMonth() ",options.date.getMonth());
...
//                                    console.log(month+"0 options.date ",options.date);
//                                    console.log("000 options.date.getMonth() ",options.date.getMonth());
//                                    
                                    
                                    options.date.setDate(1);options.date.setMonth(options.date.getMonth() + 1);
//                                    console.log("1 options.date ",options.date);
//                                    console.log("111 options.date.getMonth() ",options.date.getMonth());
                                    break;
                                case 'week':
                                    options.date.setDate(options.date.getDate() + 7);
...
//                        if(param_uid == roomuser.useruid && selected_gx_coupon.id == roomuser.couponid)
                        ...
                        if (getDDay(getTotalEndtime(selected_gx_coupon)) >= 0)
                        if(isNowTimeMinOver(etime) > 0){
    ...
    function setGXAllCount(selected_gx_coupon,allusecount){
        if(!selected_gx_coupon)return;
       //param_uid , 
        var uid_couponid = param_uid+"_"+selected_gx_coupon.id;
    ...
    function isCouponArea(allcoupons,mdate){
        ...
        for(var i = 0 ; i < allcoupons.length; i++){
            ...
            //if(compare_date(coupon.starttime, mdate) <= 0 && compare_date(coupon.endtime, mdate) >= 0){
...
//        clog("insertCalenderData ",datas);
    ...
    function reload_calendar_page(centercode){
        ...
        refresh_page("?centercode="+param_centercode+"&uid="+param_uid+"&userid="+param_userid+"&username="+param_username+"&day="+param_day); 
    }
...
//        var starttime = "";
//        var endtime = "";
//        var useruid = param_uid;
//        var userid = param_userid;
//       
//        if(intype == 0 || intype == 5){
//            nowgxcoupon = get_user_gxcoupondata(intype,roomid);
//            if(!nowgxcoupon){
    ...
    function get_user_gxcoupondata(intype,roomid){ // type 추가  //intype  //  0 : 예약하기 , 2 : QR 출석완료  , 3 : 예약취소 , 5 : 대기신청
        ...
        for(var i = 0 ; i < membership.length; i++){
            if(membership[i].mbstype == TYPE_GX){
                var maxcount =  getMbsMaxCount(membership[i]);
                ...
                var isfulluse = intype == 0 && usecount >= maxcount ? true : false;
