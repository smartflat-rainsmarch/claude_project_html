...
<!--    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>-->
    ...
    <link href='http://fonts.googleapis.com/css?family=Montserrat' rel='stylesheet' type='text/css'> <!--Montserrat 폰트설정-->
    <script src="js/scripts.js?ver3.00aaa3"></script>
        ...
        <div id="id_hcalendar" style="width:100%;">
            <div style="height:50px">
                <span style="float:left;margin-top:5px">
                    ...
                    <svg style="width:13px;height:13px;"><rect width="13" height="13" rx="3" ry="3" style="fill:#33a186"></rect></svg>&nbsp;<text style="font-size:13px; color:#3f4254;font-weight:500;">오픈됨</text>&nbsp;&nbsp;
                    <svg style="width:13px;height:13px"><rect width="13" height="13" rx="3" ry="3" style="fill:#fa6374"></rect></svg>&nbsp;<text style="font-size:13px; color:#3f4254;font-weight:500;">예약꽉참</text>&nbsp;&nbsp;
                    <svg style="width:13px;height:13px"><rect width="13" height="13" rx="3" ry="3" style="fill:#949494"></rect></svg>&nbsp;<text style="font-size:13px; color:#3f4254;font-weight:500;">비활성됨</text>&nbsp;&nbsp;
                    <svg style="width:8px;height:8px"><rect width="8" height="8" rx="4" ry="4" style="fill:#f0f0f0;border:1px solid #111111"></rect></svg>&nbsp;<text style="font-size:13px; color:#3f4254;font-weight:500;">오픈시간 확정</text>&nbsp;&nbsp;
                    <svg style="width:8px;height:8px"><rect width="8" height="8" rx="4" ry="4" style="fill:#29977a;"></rect></svg>&nbsp;<text style="font-size:13px; color:#3f4254;font-weight:500;">오픈시간 미지정</text>&nbsp;&nbsp;
...
<script>
    ...
    var param_useruid = "000011";
    var param_username = "testname";
    holidayCheckNoText();
...
//    var now_groupcode = param_groupcode;
//    var now_centercode = param_centercode;
    var year_arr = [];
...
//        var keys = ["select_gxtype","select_gxmin","select_gxteacher","input_gx_max","input_opentime","input_reservationmin","input_cancel_reservationmin","select_isshow","input_gx_note"];
        ...
        switch(key){
            case 1://select_gxtype
                ...
                input_maxreservation.value = gxtype.gxmaxreservation ? gxtype.gxmaxreservation : "5";
...
//        var rdate = new Date(y, m, d, hh, mm, ss);
    ...
    function getRoomdata(yearmonthnum,roomid){
        var return_roomdata = null;
        if(!yearmonthnum || !roomid)
            return null;
        for(i = 0; i < allgxreservation.length; i++) {
            var roomdatas = JSON.parse(allgxreservation[i].gx_roomdata);
            if(allgxreservation[i].gx_yearmonthnum == yearmonthnum){
...
//                 objs.push({"value":hourtxt,"text":hourtxt,"imgname":"","optionbg":optionbg,"textcolor":textcolor});
        ...
        if(roomid){
            var roomdata = getRoomdata(yearmonthnum,roomid);
...
//             clog("111 gxtype ",gxtype);
            ...
            var rmaxreservation = gxtype ? parseInt(gxtype.gxmaxreservation) : 0;  // GX 최대예약횟수는 모든 GX 통합이다.
        }
...
//                var param = "?centercode="+calendar_data.centercode;
            ...
            if (code == 100) {
//                    refresh_page(param);
                 C_showToast("testid2", "알림", "강좌 정보를 업데이트하였습니다.", function() {});
...
(function ($) {
  ...
  function calendar($el, options) {
    ...
    $el.on('click', '.js-cal-prev', function () {
        ...
        btn_insert_room.style.display = options.mode == "day" ? "block" : "none";
...
//                console.log("user.id" +user.userid); 
                userstext.push({"name":user.username,"userid":user.userid,"uid":user.useruid,"couponid":allusers[uid_couponid].couponid,"couponname":allusers[uid_couponid].mbsname,"maxcount":allusers[uid_couponid].maxcount,"usecount":allusers[uid_couponid].usecount,"status":user.status,"starttime":user.starttime,"endtime":user.endtime});
            }
            ...
            var type = 0;
            var insertuser_tag = users.length >= roommax ? "<div style='margin-top:10px;margin-right:2px'><i class='fa-solid fa-xmark'  onclick='removeRoom(\"" +yearmonthnum + "\",\"" +roomid + "\",\"" +hour + "\",\"" +roomname + "\",\"" +userslen + "\")' style='color:gray;padding:13px 19px 13px 19px;cursor:pointer'  title='선택한 강좌를 삭제합니다.'></i></div>" : "<div style='position:relative;margin-top:10px;margin-right:2px'><text>회원삽입:</text>&nbsp;<input class='searchBox' id='"+roomid+"' name='"+type+"' style='width:160px;height:35px;border:1px solid #ebedf3;border-radius:5px;outline:none'>&nbsp;<i class='fa-solid fa-xmark'  onclick='removeRoom(\"" +yearmonthnum + "\",\"" +roomid + "\",\"" +hour + "\",\"" +roomname + "\",\"" +userslen + "\")' style='color:gray;padding:13px 19px 13px 19px;cursor:pointer'  title='선택한 강좌를 삭제합니다.'></i></div>";
...
//            clog(sdate+" "+rsh+":"+rsm+" ~ "+reh+":"+rem+" ");
                    ...
                    if(uinfo){
                        ...
                        var tag = "<div style='"+border_top_tag+"border-bottom:1px solid #ebedf3;margin-right:10px;height:100%;background-color:"+bgcolor+"'>"+
                                    "<span style='float:left'>"+
                                        "<label  onclick='showUserInfoPopup(\"" +uinfo.userid + "\")'  style='font-size:14px;font-weight:400;margin-top:10px;margin-left:10px;cursor:pointer' title='"+desc+"'>"+(num+1)+".&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+uinfo.name+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;"+uinfo.couponname+" (<span style='color:#2194f3'>"+uinfo.usecount+"</span>/"+uinfo.maxcount+")</label>"+
                                    "</span>"+
                                    "<span style='float:right'>"+
                                        "<i class='fa-solid fa-xmark' style='color:gray;padding:10px;cursor:pointer'  onclick='remove_inuser(\"" +yearmonthnum + "\",\"" +roomid + "\",\"" +yy + "\",\"" +mm + "\",\"" +dd + "\",\"" +hour + "\",\"" +roomname + "\",\""+uinfo.name+"\",\""+uinfo.uid+"\",\"gxpage\")' ></i>"+
                                    "</span>"+
                                  "</div>";
            ...
            div.innerHTML = //타이틀 DIV
                            "<div id='top_div_"+hour+"' style='width:1120px;height:60px;background-color:"+title_color+";border:1px solid #ebedf3;border-radius:10px 10px 0px 0px'>"+
                                "<span style='float:left;'>"+
                                    "<div align='center'  onclick='roomclick( \""+dateFormat(event.start)+"\", "+yearmonthnum+" , \""+roomid+"\"  )' style='width:50px;height:40px;margin-top:10px;border-right:1px solid #ebedf3'><i class='fa-solid fa-pen-to-square' style='margin-top:8px;font-size:20px;cursor:pointer;'></i></div>"+
                                "</span>"+
                                "<span >"+
...
//        var roomid = event.roomdata.room_id;
        ...
        if(dateclass == sdate)
        {
           ...
           // var text = room.room_name+"<br>"+users.length+"/"+room.room_max+"명<br>강사명:"+room.room_managername+", 오픈시간:"+hour+"<br>Note : "+room.room_note;
            ...
            var isopentime_open = isNowTimeMinOver(room.room_opentime) <= 0 ? true : false;
            var roombg =  ismax ? "#fa6374" : "#309e83";
            if(parseInt(room.room_isshow) == 0 || !isopentime_open)roombg = "#949494";
            var div = document.createElement("div");
            div.id = roomid;
            
            var isopentime_color = room.room_opentime == "" ? "#29977a" : "#f0f0f0";
            //시간 AM 10 , PM 4
            var time = event.start.toTimeString();

            var timetag = "<text class='fmont' style='font-size:7px; color:"+isopentime_color+";font-weight:500;margin-top:7px;margin-left:3px'>●</text><label class='fmont' style='font-size:11px; color:#ffffff;text-align:left; font-weight:500;margin-top:8px;margin-left:3px'>"+time+"</label>";
            var maxuser_tag = "<text class='fmont' style='float:right;font-size:12px; color:#f0ff00;text-align:right; font-weight:600;margin-top:7px;margin-right:5px'>"+users.length+"/"+room.room_max+"</text>";
            var maintext = "<text style='position:absolute;float:left;font-size:12px; color:#ffffff;text-align:left; font-weight:700;margin-top:5px;margin-left:56px'>"+text+"</text>";
...
//         clog("==================");
            ...
            for(var i = 0 ; i < users.length;i++){
                var user = users[i];
                var uid_couponid = user.useruid+"_"+user.couponid;                
                userstext.push({"name":user.username,"uid":user.useruid,"userid":user.userid,"couponid":allusers[uid_couponid].couponid,"couponname":allusers[uid_couponid].mbsname,"maxcount":allusers[uid_couponid].maxcount,"usecount":allusers[uid_couponid].usecount,"status":user.status,"starttime":user.starttime,"endtime":user.endtime});
            }
        //시간 태그 ex: 12:00 ~ 13:20
            var endhour = (hour+Math.floor((roommin+lesson_time)/60))%24;
            var endmin = (roommin+lesson_time)%60;            
...
//            $.each(options.data, dayAddEvent);
  ...
  ;(function (defaults, $, window, document) {
    ...
    function readyUsersClick(str_ready_users,roomid){
        ...
        var type = 5;
        var user_search = "<div style='position:relative;margin-right:2px'><text style='font-weight:lighter; font-size: 18px; vertical-align: text-top;'>대기회원삽입:</text>&nbsp;<input class='searchBox' id='"+roomid+"' name='"+type+"' style='width:160px;height:35px;border:1px solid #ebedf3;border-radius:5px;outline:none;'>";
...
//        div_member_table.innerHTML = "<table class='table table-bordered' id='dataTable' width='100%' cellspacing='0'></table>";
        ...
        if (len > 0) {
            for (var i = 0; i < len; i++) {
                ...
                var type_num = 4;
                bcell_delete.innerHTML = "<button style='width:70px; height:35px; border-radius:5px;background: rgb(0, 123, 255);color: rgb(238, 238, 238);padding-left: 20px;padding-right: 20px;border: 0px;' onclick='send_remove_user(\""+str_row+"\",\""+type_num+"\")'>삭제</button>";
    ...
    function send_remove_gxuser(_rows,intype,callback){
        ...
        var nowcentercode = param_centercode;
    ...
    /* 221031 유진 수정 */
    function get_user_gxcoupondata(uid, intype){ // type 추가  //intype  //  0 : 예약하기 , 2 : QR 출석완료  , 3 : 예약취소 , 5 : 대기신청
        var info = null;
        for(var i = 0; i < gxuserinfos.length; i++){
            if(gxuserinfos[i].mem_uid == uid){
                info = gxuserinfos[i];
        ...
        for(var i = 0 ; i < membership.length; i++){
            if(membership[i].mbstype == TYPE_GX){
                var maxcount =  parseInt(membership[i].mbsmaxcount);
                ...
                //날짜가 안지났다면
                //if(getDDay(membership[i].starttime) <= 0 &&  getDDay(membership[i].endtime) >= 0)
				if(getDDay(membership[i].endtime) >= 0){
...
//                if(gxcoupon)clog(i+" gxcoupon.starttime "+ gxcoupon.starttime+" membership[i].starttime "+membership[i].starttime+" compare "+compare_date(gxcoupon.starttime,  membership[i].starttime)+" dday "+getDDay(endtime));
    ...
    function send_gxuser(roomid,uid,type){ // type 추가
        ...
        var value = get_user_gxcoupondata(uid,type); // type 추가
        clog("value is ",value);
        if(!value){
        ...
        CallHandler("adm_get", _data, function (res) {
            ...
            if (code == 100) {
                 ...
                 C_showToast("testid2", "알림", res.message, function() {});
...
//                    btn_insert_room.style.display = window.options.mode == "day" ? "block" : "none";
//                    first_goto_day = true;
//                }
//            },100);
//        }
