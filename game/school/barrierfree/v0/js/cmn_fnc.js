...
function C_AddCustomFont(parent, str, x, y, tag, scale, anchor, fontsize, color, isboldfont, onClick) {
    var labelFont = isboldfont ? "현대하모니B" : "현대하모니L";
    var lines = str.split('\n');
...
//문자개수로만 줄바꿈
function replaceStrToEnterMaxLen(str, maxlen) {
    if (!str || !maxlen || maxlen <= 0) return str;
...
function addSpriteInText(txtdata,sp){
    ...
    if (txtdata && txtdata.message) {
        ...
        var _str = txtdata.maxlen ? replaceStrToEnterMaxLen(txtdata.message, txtdata.maxlen) : replacePLineToEnter(txtdata.message);
        var label = new cc.LabelTTF( _str, fontName, fontSize );
