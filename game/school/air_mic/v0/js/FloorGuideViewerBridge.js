  ...
  class FloorGuideViewerBridge {
    ...
    _emit(event, data) {
      ...
      this._eventListeners[event].forEach(callback => {
        try {
          ...
          console.error(`[FloorGuideViewerBridge] Event handler error (${event}):`, err);
        }
      });
    }
    ...
    _updateSearchTrie() {
      try {
        ...
        allPOIs.forEach((poi, index) => {
          ...
          // 키워드 배열을 공백으로 연결하여 검색 가능하게
          const keywords = (poi.properties?.keywords || []).join(' ');

          // trie_floors 형식: {idx, name, data, floor}
          // data 형식: "층이름|POI이름|카테고리|설명|키워드"
          const dataStr = floorName + '|' + name + '|' + category + '|' + description + '|' + keywords;
          const trieData = {
            idx: index,
            name: name,
            data: dataStr,
    ...
    _setupEventListeners() {
      ...
      this.viewer.on('click', () => {
        ...
        // 약간의 딜레이 후 체크 (poi:click이 먼저 발생하도록)
        setTimeout(() => {
          // POI 클릭 후 300ms 이내면 빈 공간 클릭으로 처리하지 않음 (터치 이벤트 타이밍 이슈 방지)
          const timeSincePoiClick = Date.now() - (this._poiClickedTime || 0);
          if (timeSincePoiClick > 300) {
    ...
    _handlePOIClick(poiInfo) {
      ...
      // 타이밍 이슈 방지를 위해 timestamp 사용
      this._poiClickedRecently = true;
      this._poiClickedTime = Date.now();
    ...
    setFloor(floorIdentifier) {
      ...
      if (floorId) {
        // 층 변경 전에 팝오버 닫기 (추가 안전장치)
        if (this.viewer.isPopoverVisible && this.viewer.isPopoverVisible()) {
          this.viewer.hidePopover();
        }

        this.viewer.setFloor(floorId);
        this.currentFloorId = floorId;
      } else {
        console.warn('[FloorGuideViewerBridge] Could not resolve floorId for:', floorIdentifier);
      }
    }
          ...
          closeBtn.addEventListener('click', () => {
            ...
            if (this.viewer) {
              ...
              this.viewer.clearHighlight();
                ...
                //neel add
                if(txtguide_bg)txtguide_bg.removeAllChildren();
            }
          });
        }
      }, 0);
    ...
    async _navigateToSelectedPOI() {
      ...
      try {
        ...
        if (route) {
          ...
          // 상세 경로 안내 정보 출력
          const detailedResult = this.viewer.getDetailedRouteInstructions(
            route,
            currentLocation.name || '현재 위치',
            this.selectedPOI.name
          );
          console.log('[Route Instructions] 상세 경로 안내:', detailedResult);
          console.log('[Route Instructions] 요약:', detailedResult.summary);
          console.log('[Route Instructions] 단계별 안내:');
          detailedResult.instructions.forEach((inst) => {
            const turnInfoStr = inst.turnInfo
              ? ` [${inst.turnInfo.clockDirection}시 방향, 각도: ${Math.round(inst.turnInfo.angle)}°]`
              : '';
            console.log(`  ${inst.step}. [${inst.action}] ${inst.description}${turnInfoStr}${inst.distance ? ` (${Math.round(inst.distance / 100)}m)` : ''}`);
          });

          // 길찾기 성공 이벤트 발생
          this._emit('navigate:success', {
            poi: this.selectedPOI,
            from: currentLocation,
            route: route,
            instructions: detailedResult
          });
...
getFloorByTabIndex(tabIndex) {
  ...
  async function autoInit() {
    ...
    // 길찾기 버튼 클릭 이벤트 리스너 등록
    global.floorViewerBridge.on('navigate:click', (data) => {
      console.log('[Navigate Event] 길찾기 버튼 클릭');
      console.log('[Navigate Event] 출발지:', data.from?.name || '현재 위치');
      console.log('[Navigate Event] 목적지:', data.poi?.name);
    });

    global.floorViewerBridge.on('navigate:success', (data) => {
      console.log('[Navigate Event] 길찾기 성공');
      console.log('[Navigate Event] 경로:', data.route);
      console.log('[Navigate Event] 안내:', data.instructions?.summary);
    });

    global.floorViewerBridge.on('navigate:error', (data) => {
      console.error('[Navigate Event] 길찾기 실패:', data.error);
    });
  }
