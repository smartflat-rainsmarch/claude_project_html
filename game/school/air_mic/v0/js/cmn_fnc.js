...
function C_AddCustomFont_Back(parent, str, x, y, tag, scale, anchor, fontsize, color, isboldfont, onClick) {
    ...
    if (isboldfont != undefined && isboldfont) {
        label = cc.LabelTTF.create(str, "ChosunBg", fontsize);
    } else {
        label = cc.LabelTTF.create(str, "ChosunSg", fontsize);
    }
...
function C_AddCustomFont(parent, str, x, y, tag, scale, anchor, fontsize, color, isboldfont, onClick) {
    var labelFont = isboldfont ? "LatoBold" : "Lato";
    var lines = str.split('\n');
    ...
    if (lines.length > 1) {
        ...
        var label = cc.LabelTTF.create(str, labelFont, fontsize);
        label.setPosition(x, y);
...
function C_AddCustomFontNew(parent, str, x, y, tag, scale, anchor, fontsize, color, isboldfont, onClick, isradius) {
    ...
    if (anchor != null){
        label.setAnchorPoint(cc.p(anchor / 2, 0.5));
        // 2. [추가] 텍스트 내부 정렬 설정 (줄바꿈 시 정렬 문제 해결)
        // anchor가 0이면 왼쪽, 1이면 가운데, 2면 오른쪽으로 가정
        if (anchor == 1) {
...
function C_AddCustomFontSprite(parent, str, x, y, tag, scale, anchor, fontsize, color, isboldfont, callback, isradius, isradiusshow) {
    var label = isboldfont
        ? cc.LabelTTF.create(str, "ChosunBg", fontsize)
        : cc.LabelTTF.create(str, "ChosunSg", fontsize);
...
function C_addInputBox(parent, x, y, width, height, placeholder, fontSize, onChangeListener) {
    var bg = new cc.Scale9Sprite("res/empty.png");
    var editBox = new cc.EditBox(cc.size(width, height), bg);
    editBox.setPosition(x, y);

    editBox.setPlaceHolder(placeholder || "입력하세요");
    editBox.setPlaceholderFontSize(fontSize || 28);
    editBox.setFontSize(fontSize || 28);
    editBox.setFontColor(cc.color(30, 30, 30));
    editBox.setPlaceholderFontColor(cc.color(160, 160, 160));
    ...
    // 델리게이트 객체 설정
    var delegate = cc.EditBoxDelegate.extend({
        editBoxEditingDidEnd: function (sender) {
            if (typeof onChangeListener === "function") {
//                onChangeListener(sender.getString());
            }
        },
        editBoxTextChanged: function (sender, text) {
            onChangeListener(text);
        },
        // 필요한 경우 다른 이벤트도 구현 가능
...
//                onChangeListener(sender.getString());
    ...
    editBox.setDelegate(new delegate());

    parent.addChild(editBox, 999);
    return editBox;
}
function C_AddSprite(parent, res, x, y, tag, scale, zindex, w, h) {
    // 1. Base64 여부 확인 (data:image/ 로 시작하는지 체크)
    var isBase64 = typeof res === "string" && res.indexOf("data:image/") === 0;
    var finalRes = res;

    if (isBase64) {
        // Base64는 URL 파라미터(?v=)를 붙이면 데이터가 깨지므로 원본 그대로 사용합니다.
        // 강제 새로고침이 필요하다면 textureCache에서 해당 base64 키를 직접 제거해야 합니다.
        cc.textureCache.removeTextureForKey(res);
    } else {
        // 일반 URL일 경우 기존처럼 타임스탬프 추가
        var timestamp = new Date().getTime();
        finalRes = res + (res.indexOf("?") > -1 ? "&" : "?") + "v=" + timestamp;
        cc.textureCache.removeTextureForKey(res);
    }

    // 2. 스프라이트 생성
    var sp = new cc.Sprite(finalRes);
    sp.setPosition(x, y);
    ...
    // 4. 수동 크기(w, h) 설정 (있을 경우)
    // Base64 이미지는 비동기로 로드될 수 있어, 즉시 크기 조절이 필요할 때 유용합니다.
    if (w && h) {
        var contentSize = sp.getContentSize();
        if (contentSize.width > 0) {
            sp.setScaleX(w / contentSize.width);
            sp.setScaleY(h / contentSize.height);
        }
    }
...
function C_ShowSpeak(callback){
    console.log("C_ShowSpeak 000");
    if(NOW_SELF){
//        console.log("C_ShowSpeak 111");
//        if(window.android && window.android.isWakewordLoaded() == 1){
//            window.floorViewerBridge.hide();
//            console.log("C_ShowSpeak 222");
//            C_ShowSpeakDialogLayer(NOW_SELF, "음성인식" , "음성으로 말해주세요.", function(){
//                window.floorViewerBridge.show();
//            });
//        }else{
//            C_Toast(NOW_SELF, "마이크 음성인식 설정중입니다...");
//        }
...
var speak_dialoglayer = null;

function C_ShowSpeakDialogLayer(self, title , message, OKCallback , CancelCallback, style){
    // 1. 이미 레이어가 생성되어 있다면 부모로부터 제거하고 초기화
    if (speak_dialoglayer != null) {
        // cc.sys.isObjectValid는 객체가 메모리상에 유효한지 체크하는 안전한 방법입니다.
        if (cc.sys.isObjectValid(speak_dialoglayer)) {
            speak_dialoglayer.removeFromParent(true); // true는 cleanup 실행 여부
        }
        speak_dialoglayer = null;
        
    }
    if(self.hideWebView)self.hideWebView();
    
    // 2. 새로운 레이어 생성 및 추가
    speak_dialoglayer = new SpeakDialogLayer();
    speak_dialoglayer.setTag(ID.POPUP_LAYER_DIALOG);

    self.addChild(speak_dialoglayer, 2222);
    speak_dialoglayer.init(title, message, OKCallback, CancelCallback, style);
}
function C_ShowDialogLayer(self, title , message, imgres, OKCallback , CancelCallback,style){
...
function C_ShowMapDialogLayer(self, title , message, mapdatas, OKCallback , CancelCallback,style){
    console.log('[C_ShowMapDialogLayer] Called with title:', title, 'message:', message);
    console.log('[C_ShowMapDialogLayer] window.floorViewerBridge:', window.floorViewerBridge);

    // Floor Guide Viewer 사용 (우선)
    if (window.floorViewerBridge) {
        console.log('[C_ShowMapDialogLayer] Using Floor Guide Viewer');
        try {
            // title에서 층 정보와 검색어 추출 (예: "1층|스타벅스" 또는 "스타벅스")
            var searchQuery = '';
            var floorId = null;
...
function initIframeTouchEvent(self){
    window.addEventListener("message", function(event) {
            console.log("📩 iframe 메시지 수신됨:111", event.data);
...
function drawAirText(parent, x, y, mode, dustInfo){
   ...
   if(!parent)return;
    ...
    if(mode == "icon"){
        ...
        C_AddCustomFont(parent,"미세먼지", x, y+30, null, 1, cc.TEXT_ALIGNMENT_CENTER , 30, C_HEXtoCCColor("#ffffff"),false);    
        if(_icon)C_AddSprite(parent,_icon, x, y-30,1,1,null);    
    }else { // text mode
        var _grade = dustInfo.grade ? dustInfo.grade : "";
        C_AddCustomFont(parent,"미세먼지", x, y+30, null, 1, cc.TEXT_ALIGNMENT_CENTER , 40, C_HEXtoCCColor("#ffffff"),false);    
        if(_grade)C_AddCustomFont(parent,_grade, x, y-20, null, 1, cc.TEXT_ALIGNMENT_CENTER , 35, C_HEXtoCCColor("#ffffff"),false);    
    }
...
function drawWeatherToday(parent, x, y, weatherdata, spdata){//mode, _fontColor, _fontSizeBig, _fontSizeSmall,iconscale){
     ...
     C_AddCustomFont(parent,weatherdata.today_tmp+"℃", x, y+30, null, 1, cc.TEXT_ALIGNMENT_CENTER , 60, C_HEXtoCCColor(spdata.fontcolor),false);    
     C_AddSprite(parent,getWeatherIconRes(weatherdata.today_sky, weatherdata.today_pty), x+150, y+30,1,spdata.iconscale,null);    
     C_AddCustomFont(parent,"최저기온"+weatherdata.today_tmn+"℃", x, y-30, null, 1, cc.TEXT_ALIGNMENT_CENTER , 20, C_HEXtoCCColor(spdata.fontcolor),false);    
     C_AddCustomFont(parent,"최고기온"+weatherdata.today_tmx+"℃", x+150, y-30, null, 1, cc.TEXT_ALIGNMENT_CENTER , 20, C_HEXtoCCColor(spdata.fontcolor),false);    
...
function C_loadJSON(filePath, callback) {
    fetch(filePath)
        .then(response => {
            if (!response.ok) {
...
function C_spriteAnimation(parent, x, y, res_arr, speed, scale, onClickCallback) {
    // 애니메이션 생성
    ...
    var listener = cc.EventListener.create({
        event: cc.EventListener.TOUCH_ONE_BY_ONE,
        swallowTouches: true, // 하위 터치 이벤트 차단
        onTouchBegan: function (touch, event) {
            var target = event.getCurrentTarget();
            var locationInNode = target.convertToNodeSpace(touch.getLocation());
            ...
            // 터치가 스프라이트 영역 안에 있는지 확인
            if (cc.rectContainsPoint(rect, locationInNode)) {
                if (typeof onClickCallback === "function") {
                    onClickCallback(target); // ✅ 콜백 실행
                }
                return true; // 터치 이벤트 처리됨
            }
            return false;
        }
    });
...
function C_setUI(self,size,data){
    var sp = null;
    var ani_sps = [];
    if(!self.language_code)self.language_code = "KO";
            ...
            if (data.texttype) {
                ...
                switch (data.texttype) {
                    ...
                    case "hh:mm":
                        ...
                        var hh = date.getHours();                        // 12시간제로 변환
...
//                        mtext = hour + ":" + minute;
    ...
    if(self.all_id_sps)self.all_id_sps.push({"imgurl":C_getResPath(data.imgurl), "sp":sp});
    return sp;
}
...
//        console.log("nowtime ",nowtime);  
        ...
        var date = new Date();
        md.string = (date.getMonth()+1)+"/"+date.getDate();
//        hhmm.string = now_sectime%2 == 0 ? date.getHours()+" "+date.getMinutes() : date.getHours()+":"+date.getMinutes();
        hhmm.string = date.getHours()+":"+date.getMinutes();
        
        var weekdays = ["(일)", "(월)", "(화)", "(수)", "(목)", "(금)", "(토)"];
        console.log("weekdays[date.getDay()] "+weekdays[date.getDay()]);
...
function C_updateAir(sp, data){
   ...
   if(before_airhour == -1 || before_airhour != h){
        ...
        getAirData(function(airdata_str){
            ...
            if(now_airdata.response && now_airdata.response.body && now_airdata.response.body.items && now_airdata.response.body.items.length > 0){
...
//누근가 접근했다면 현재 화면에서 접근했을때 화면으로 보여준다.
var IDLE_TIME_LIMIT = 60 * 1000; // 1분 (밀리초 단위)

function C_CheckGPio(self) {
    var currentTime = (new Date()).getTime();

    // 1. 센서 신호가 있었는지 확인
    if (isPIRSensor) {
        // 2. 마지막 고객 클릭(사용)으로부터 1분이 지났는지 확인
        // 만약 고객이 사용 중(클릭 후 1분 이내)이라면 센서 신호를 무시하고 리턴합니다.
        if (currentTime - userClickTimestamp < IDLE_TIME_LIMIT) {
            console.log("고객이 사용 중입니다. 센서 신호를 무시합니다.");
            isPIRSensor = false; // 신호 초기화만 하고 이동은 안 함
            return;
        }

        // 3. 1분이 지났고 센서 신호가 들어왔다면 홈으로 이동
        isPIRSensor = false;
        if (NOW_SCENE != SCENE_HOME) {
            console.log("사용 종료 후 1분 경과 및 새 사용자 감지: 홈으로 이동");
            idleMode = IDLE_SPEAK;
...
//PIR 센서에 왔다면 소리를 낸다.
function C_CheckGPioSpeak(self){
    if(idleMode == IDLE_SPEAK){
        idleMode == IDLE_OFF;
        C_PlaySound(SOUND.SIGN);
        C_PlaySound(SOUND.HELLO);
...
//        C_playTTS("안녕하세요. 무엇을 도와드릴까요?");
    }
    }
function C_playTTS(text){
     if(window.android)window.android.playTTS(text);
}
function C_ChangeSpriteByTag(parent, tag, newRes) {
    var targetSp = parent.getChildByTag(tag);
    if (targetSp instanceof cc.Sprite) {
        C_ChangeSpriteImage(targetSp, newRes);
    }
}
/**
 * 기존 스프라이트의 이미지만 교체하는 함수
 * @param {cc.Sprite} targetSp 교체할 대상 스프라이트 객체
 * @param {string} newRes 새로운 이미지 경로
 */
function C_ChangeSpriteImage(targetSp, newRes) {
    if (!targetSp || !cc.sys.isObjectValid(targetSp)) {
        cc.log("대상 스프라이트가 유효하지 않습니다.");
        return;
    }

    var timestamp = new Date().getTime();
    var res_url = newRes + "?v=" + timestamp;

    // [수정] 텍스처를 추가할 때 콜백에서 처리하는 것이 가장 확실합니다.
    cc.textureCache.addImage(res_url, function(texture) {
        if (texture) {
            // 1. 텍스처 적용
            targetSp.setTexture(texture);
            
            // 2. [핵심] 그리기 영역 갱신 
            // 이 코드가 없으면 새 이미지의 크기가 다를 때 화면에 나타나지 않거나 잘려 보입니다.
            var size = texture.getContentSize();
            targetSp.setTextureRect(cc.rect(0, 0, size.width, size.height));
            
            // 3. 캐시 관리 (선택 사항: 메모리 누수 방지)
            // 다음 교체를 위해 현재 로드된 URL 키를 제거
            // cc.textureCache.removeTextureForKey(res_url);
            
            cc.log("이미지 교체 및 렉트 갱신 완료: " + size.width + "x" + size.height);
        } else {
            cc.log("텍스처 로드 실패: " + res_url);
        }
    }, this);
}
// 사용 예
C_ChangeSpriteByTag(this, 10, "res/new_image.png");
...
//서버에 보낼 가공된 층별정보 POI만 추출
function initFloorInfos(){
    const allFloors = window.floorViewerBridge.viewer.getAllFloorsData();
    console.log("allFloors ",allFloors);
    now_floor_info = {};
    now_floor_info["floordata"] = [];
    
    for(var i = 0 ; i < allFloors.length; i++){
        var fdata = allFloors[i];
        var floordata = {};
        floordata["id"] = fdata.id;
        floordata["level"] = fdata.level;
        floordata["fname"] = fdata.name;
        floordata["desc"] = fdata.description;
        floordata["floorbg"] = fdata.backgroundImage ? fdata.backgroundImage.filename : "";
        floordata["width"] = fdata.backgroundImage ? fdata.backgroundImage.orignalWidth : 0;
        floordata["height"] = fdata.backgroundImage ? fdata.backgroundImage.orignalHeight : 0;
        floordata["data"] = [];
        
        
        var elements = allFloors[i].elements;
        var new_elements = [];
        for(var j = 0 ; j < elements.length; j++){
            
            if(elements[j].type == "POI"){
                var blockData = {};
                blockData["id"] = elements[j].id ? elements[j].id : "";
                blockData["title"] = elements[j].properties ? elements[j].properties.name : "";
                blockData["desc"] = elements[j].properties ? elements[j].properties.description : "";
                blockData["x"] = elements[j].points ? elements[j].points[0] : "";
                blockData["y"] = elements[j].points ? elements[j].points[1] : "";
                blockData["event"] = {};
                blockData["event"]["img"] = elements[j].properties ? elements[j].properties.description : "";
                
                floordata["data"].push(blockData);
                
            }
        }
        now_floor_info["floordata"].push(floordata);
    }
    
    
    console.log("서버에 보낼 가공된 층별정보 POI만 추출 : ",now_floor_info);
    
//    var ftest0 = {
//          "id": "792860ff-3d1f-4b3c-b5cb-192d47418843",
//          "name": "지하1층",
//          "level": 0,
//          "description": "시설안내",
//          "backgroundImage": {
//            "filename": "images/floor-지하1층.png",
//            "embedded": true,
//            "base64": null,
//            "originalWidth": 680,
//            "originalHeight": 941,
...
//
//            {
//              "id": "8b36b013-d3d6-427a-a2e8-4cfb300c5f5f",
//              "type": "POI",
//              "points": [
//                190,
//                410
...
//                "description": "구내식당\n운영시간 : 8:00 ~ 20:00",
//                "markerShape": "circle",
//                "customIconId": "md/MdOutlineRestaurant",
//                "customIconName": "식당",
...
////        "type": "floor",
////        "floortitle": "층별 안내",
////        "floortitlebg": "res/global/floor_title_bg.png",
////        "desc": "사이즈는 2160x3142 기준이다 2배가 크다",
////        "width": 1080,
////        "height": 883,
//        "floordata": [
//            {
//                "fname": "1F",
//                "height": "192",
//                "floorbg": "contentdata/img/map1.png",
//                "data": [
//
//                    {
//                        "title": "진단검사의학과",
...
//                            "type": "popup",
//                            "message": "",
//                            "img": "contentdata/floor/진단검사의학과 길안내(1층 키오스크).png"
//                        }
//                    }
//                ]
//            }
...
//    }
//    
    return now_floor_info;
}
/**
 * SVG 문자열을 Base64 Data URI로 변환하는 함수
 */
function svgToBase64(svgString) {
    // 1. 유니코드 문자 처리를 위해 인코딩
    const utf8String = unescape(encodeURIComponent(svgString));
    // 2. btoa 함수를 사용하여 Base64 변환
...
function C_createScrollableSpriteList(parent, sps, mx,my, areaWidth, areaHeight, itemWidth, itemHeight, gapX = 10, gapY = 10) {
    // ScrollView 생성
    const scrollView = new cc.ScrollView();
    scrollView.setViewSize(cc.size(areaWidth, areaHeight));
    ...
    // 내용 레이어 생성
    const maxCols = Math.floor((areaWidth + gapX) / (itemWidth + gapX));
    const totalRows = Math.ceil(sps.length / maxCols);
    const contentHeight = totalRows * (itemHeight + gapY);

    const contentLayer = new cc.Layer();
    contentLayer.setContentSize(cc.size(areaWidth, contentHeight));
    ...
    for (let i = 0; i < sps.length; i++) {
        ...
        const posX = x * (itemWidth + gapX) + itemWidth / 2;
//         const posX = (areaWidth-itemWidth)/2;
        const posY = contentHeight - (y * (itemHeight + gapY)) - 40;

        sp.setPosition(posX, posY);
        contentLayer.addChild(sp);

        x++;
    }

    // 맨 위로 보이게 설정
    scrollView.setContentOffset(cc.p(0, areaHeight - contentHeight));

    // 부모에 추가 및 반환
    parent.addChild(scrollView);
    return scrollView;
}
//function C_createScrollableSpriteList(parent, sps, areaWidth, areaHeight, itemWidth, itemHeight, gapX = 10, gapY = 10) {
//    // 1. ScrollView 생성
//    const scrollView = new cc.ScrollView();
//    scrollView.setViewSize(cc.size(areaWidth, areaHeight));
//    scrollView.setDirection(cc.SCROLLVIEW_DIRECTION_VERTICAL);
...
//    
//    // 위치 설정 (사용자 설정값)
//    scrollView.setPosition(-100, 0);
//
//    // --- [시작] 영역 확인용 가이드라인 추가 ---
//    
//    // 1) 배경색 레이어 (반투명 빨간색으로 영역 채우기)
//    const guideBg = new cc.LayerColor(cc.color(255, 0, 0, 50)); // R:255, G:0, B:0, Alpha:50
//    guideBg.setContentSize(cc.size(areaWidth, areaHeight));
//    scrollView.addChild(guideBg, -1); // 아이템보다 뒤에 배치
//
//    // 2) 테두리 선 (두꺼운 노란색 선으로 테두리 그리기)
//    const borderNode = new cc.DrawNode();
//    const vertices = [
//        cc.p(0, 0),
//        cc.p(areaWidth, 0),
//        cc.p(areaWidth, areaHeight),
//        cc.p(0, areaHeight)
//    ];
//    // drawPoly(정점배열, 채우기색, 선두께, 선색상)
//    borderNode.drawPoly(vertices, cc.color(0, 0, 0, 0), 3, cc.color(255, 255, 0, 255));
//    scrollView.addChild(borderNode, 1000); // 가이드를 잘 보이게 가장 위에 배치
//
//    // --- [종료] 영역 확인용 가이드라인 추가 ---
//
//    // 3. 내용 레이어 생성
//    const maxCols = Math.floor((areaWidth + gapX) / (itemWidth + gapX));
//    const totalRows = Math.ceil(sps.length / maxCols);
//    const contentHeight = Math.max(areaHeight, totalRows * (itemHeight + gapY));
//
//    const contentLayer = new cc.Layer();
//    contentLayer.setContentSize(cc.size(areaWidth, contentHeight));
//    
//    // 내용물 영역도 확인하고 싶다면 아래 주석 해제 (초록색)
//    /*
//    const contentGuide = new cc.LayerColor(cc.color(0, 255, 0, 30));
//    contentGuide.setContentSize(contentLayer.getContentSize());
//    contentLayer.addChild(contentGuide, -1);
//    */
//
//    scrollView.setContainer(contentLayer);
//
...
//
//        const posX = x * (itemWidth + gapX) + itemWidth / 2;
//        // ScrollView 좌표계상 상단부터 배치하기 위한 계산
//        const posY = contentHeight - (y * (itemHeight + gapY)) - (itemHeight / 2);
//
//        sp.setPosition(posX, posY);
//        contentLayer.addChild(sp);
//        x++;
...
//    return scrollView;
//}
function C_sortEvents(data) {
  const now = new Date();

  return data.sort((a, b) => {
    const startA = new Date(a.lt_startdate);
    const endA = new Date(a.lt_enddate);
    const startB = new Date(b.lt_startdate);
    const endB = new Date(b.lt_enddate);

    // 상태 구분을 위한 헬퍼 함수 (0: 진행중, 1: 예정, 2: 종료)
    const getPriority = (start, end) => {
      if (now >= start && now <= end) return 0; // 진행 중
      if (now < start) return 1;               // 미래 예정
      return 2;                                // 종료됨
    };

    const priorityA = getPriority(startA, endA);
    const priorityB = getPriority(startB, endB);

    // 1. 상태(우선순위)가 다르면 우선순위 높은 순으로 정렬
    ...
    // 2. 같은 상태 내에서의 세부 정렬
    if (priorityA === 0) {
      // 진행 중인 것끼리는 종료가 임박한 순서대로 (선택 사항)
      return endA - endB;
    } else if (priorityA === 1) {
      // 미래 일정은 시작일이 가까운 순서대로
...
function C_drawTextGuide(parent,txt_distance,txt_time,txt_move,data_guide){
        var self = this;  
        var size = cc.winSize;
        var y = 0;
        C_AddCustomFont(parent, txt_distance, 0, y-40, null, 1, cc.TEXT_ALIGNMENT_LEFT , 30, C_HEXtoCCColor("#880000"),true); //총거리   
//        C_AddCustomFont(parent,txt_time, 0, y-50, null, 1, cc.TEXT_ALIGNMENT_LEFT , 25, C_HEXtoCCColor("#000088"),false);    //예상시간
        C_AddCustomFont(parent,txt_move, 0, y-80, null, 1, cc.TEXT_ALIGNMENT_LEFT , 25, C_HEXtoCCColor("#000088"),false);  //층간이동
        
        let sps = [];
        for(var i = 0 ; i < data_guide.length; i++){
           
            let sp = new cc.Sprite(res.empty); // 이미지 자원 필요            
            var iconbg = C_AddSprite(sp,"res/global/arc.png", 0, 5,1,1,null);
            C_AddSprite(iconbg, data_guide[i].img, 20, 20,i,1.2,null);
            C_AddCustomFont(sp, C_WrapTextByWidth(data_guide[i].txt, 10), 35, 0, null, 1, cc.TEXT_ALIGNMENT_LEFT , 20, C_HEXtoCCColor("#313131"),true);              
            sps.push(sp);
        }
        var scrollView = C_createScrollableSpriteList(parent, sps, -180,-720, 420,600,420,70,2,2);
    }

//텍스트 길찾기 정보 표시
function C_floorNavigate(poiid){
     window.floorViewerBridge.viewer.navigate({
                      from: { type: 'current' },
                      to: { type: 'poi', id: poiid}
                    }).then(function(route) {
                      if (route) {
                        // 6. 경로 애니메이션 시작
                        ...
                        // 7. (선택) 상세 경로 안내 정보 조회
//                        var instructions =  window.floorViewerBridge.viewer.getDetailedRouteInstructions(
//                          route,
//                          '현재 위치',
//                          poi.name
//                        );
//                        console.log('경로 안내:', instructions);
                          ...
                          var txt_distance = '총 거리:'+parseInt(detailedResult.summary.totalDistance);
                          ...
                          C_drawTextGuide(txtguide_bg, txt_distance,txt_time,txt_move,data_guide);
                      }
