...
//         self.drawPoster();
        ...
        this.input_search = C_addInputBox(this, 500, C_getY(220), 800, 80,"검색사항을 입력하세요",26, function(text){
            ...
            for(var i = 0 ; i < arr.length; i++){
                ...
                const title = marr[1];
                const floor = marr[2];
                const tag = trie_floors[idx].data;

                console.log("tag is "+tag);
                var x = 50;
                var y = gabY+self.sp_searcharea.height -i*45;
                C_AddCustomFontNew(self.sp_searcharea, title+"("+floor+")", parseInt(x), parseInt(y), tag, 1, cc.TEXT_ALIGNMENT_LEFT ,34,cc.color(0,0,0),false,function(_tag){ 
...
//            self.before_floor_image.runAction(cc.sequence(move, onComplete));
    ...
    moveInFloorImage: function() {
        ...
        // 1. 모든 층 이미지를 화면 밖(1600)으로 일단 보냄
        for (var i = 0; i < self.floor_map_images.length; i++) {
            var sp = self.floor_map_images[i].sp;
            sp.stopAllActions(); // [중요] 실행 중인 이전 액션이 있다면 중지
            sp.setPosition(1600, sp.y);
        }

        // 2. 현재 선택된 층(now_floor)을 찾아서 부모 위치로 이동
        for (var i = 0; i < self.floor_map_images.length; i++) {
            var sp = self.floor_map_images[i].sp;
            ...
            if (self.now_floor == getStringInNumber(data.id)) {
                ...
                var move = cc.moveTo(0.5, cc.p(data.x, data.y));

                // [핵심] 가속도가 점점 줄어드는 ExponentialOut 효과 적용
                var easeMove = move.easing(cc.easeExponentialOut());

                self.before_floor_image.runAction(easeMove);
                break;
            }
        }
    },
