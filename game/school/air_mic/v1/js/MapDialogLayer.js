...
var MapDialogLayer = cc.Layer.extend({
    ...
    init:function(title,message, mapdatas, OKCallback , CancelCallback,style){
        ...
        self._bg.y =  size.height/2+240;
        self._bg.width = 1080;
        self._bg.height = 1200;
        ...
        //X 버튼
        C_AddAnimButton(self._bg, "res/n_menu/home.png" ,null, 130,1240,tag_dialog.btn_x,1,null,function(tag){
         ...
         //line 
//        C_AddSprite(self._bg, "res/n_menu/title4.png",400, 1220, 4,1.6,null);
        
        //층별안내 버튼        
        C_AddAnimButton(self._bg, "res/btn_1f.png" ,null, 160,140,tag_dialog.f1,0.7,null,function(tag){
            if(self.floor != 1){
                self.floor = 1; 
                self.buttonClick(tag);
            }
        },self);
        C_AddAnimButton(self._bg, "res/btn_2f.png" ,null, 400,140,tag_dialog.f2,0.7,null,function(tag){
            if(self.floor != 2){
                self.floor = 2; 
                self.buttonClick(tag);
            }
        ...
        var floor_btn1 = C_AddSprite(self._bg,  "res/btn_1f_on.png", 160,140,1,null,null);
        var floor_btn2 = C_AddSprite(self._bg,  "res/btn_2f_on.png", 400,140,1,null,null);
        ...
        var bottom_btn_datas = [];
        var mainData = getgamedata.hm_main_data || [];
        for(var i = 0 ; i < mainData.length;i++){
            if(mainData[i].id.indexOf("home_btn") >= 0){
                bottom_btn_datas.push(mainData[i]);
            }
        }
        
       
        var tab_size = 270;
        for(var i = 0 ; i < bottom_btn_datas.length; i++){
            var data = bottom_btn_datas[i];
             var x = data.x-185;
             var y = data.y-160;
             var w = 371;
             var h = 320;
...
//             C_AddTransparentButton(self, x,C_getY(y),w,h,tag,  function (_tag) {
//                   console.log("tag is ",_tag);
//                self._remove(_tag);
//                
//            });
                if(i != 4)
             C_AddSpriteTouch(self,C_getResPath(data.imgurl), data.x, C_getY(data.y),i,scale,null, function(tag) {
                self._remove(tag);
            });
         }
...
//        console.log("nowtime "+nowtime +" beforetime "+self.beforetime);
    ...
    clickFloorData:function(idx, floordata){
        ...
        const icon = floordata_item.icon ? floordata_item.icon : "";
        var fontsize = floordata_item.fontsize ? floordata_item.fontsize*2 : 28;
        var imgres = floordata_item.event.img;
        var color = floordata_item.fontcolor ? C_HEXtoCCColor(floordata_item.fontcolor) :C_HEXtoCCColor("#3131ff");
        if(message || imgres){
            fontsize = fontsize+6;
            color = C_HEXtoCCColor("#5566ff");
        }
        
//        var imgres = "contentdata/floor/"+fname+"_"+title+".jpg";
        console.log(floordata_item);
        console.log(title+" :: imgres "+imgres+" color is "+color);
         if(icon){
            var sp = C_AddSprite(self.mapSprite, "res/"+icon.name,  parseInt(x)+icon.ix, C_getY(parseInt(y))+icon.iy, null,1,null);
            if(icon.name == "ic_here.png")
               C_UpDownRepeatAni(sp,0.5,20);
            
        }           
        var sp = C_AddCustomFontNew(self.mapSprite, title, parseInt(x), C_getY(parseInt(y)), title, 1, cc.TEXT_ALIGNMENT_CENTER ,fontsize,color,true,function(tag){
            const _title = title.replace(/\n/g, "");            
            if(message || imgres)C_ShowDialogLayer(self, _title , message, imgres, function(){
    ...
    buttonClick:function(tag){
        ...
        // Floor Guide Viewer 사용 시 층 변경
        if (window.floorViewerBridge && window.floorViewerBridge.isInitialized) {
            console.log('[MapDialogLayer] Switching floor via FloorGuideViewer:', self.floor);
            window.floorViewerBridge.setFloor(self.floor);
        } else {
            // 폴백: 기존 Cocos2d 지도 사용
            self.initMap(self._bg, self.floor);
        }
    ...
    _remove:function(tag){
        ...
        // Floor Guide Viewer 숨기기
        if (window.floorViewerBridge && window.floorViewerBridge.isVisible) {
...
//        console.log("initWeightSpeed");
        ...
        this.zoomSlider.y = C_getY(_bg.height+430);
...
//        C_SaveData("health_speed",speed);
   ...
   initMap: function (_bg, value) {
        ...
        if (window.floorViewerBridge && window.floorViewerBridge.isInitialized) {
            console.log('[MapDialogLayer] Using Floor Guide Viewer for floor:', value);

            // Floor Guide Viewer 표시 및 층 변경
            window.floorViewerBridge.show();
            window.floorViewerBridge.setFloor(value);

            // 기존 Cocos2d 지도 요소 숨기기
            if (this.clipper) {
                this.clipper.setVisible(false);
            }

            // 층별 데이터는 여전히 로드 (검색 등을 위해)
            self.initFloorData();
            return;
        }

        // ========== 폴백: 기존 Cocos2d 지도 이미지 사용 ==========
        console.log('[MapDialogLayer] Fallback: Using Cocos2d map image');
