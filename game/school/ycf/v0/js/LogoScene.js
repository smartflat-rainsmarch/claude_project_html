...
//        this.addChild(new cc.LayerColor.create(cc.color(255, 255, 255, 255)));
        ...
        for(var i = 0 ; i < self.homedatas.length;i++){
            ...
            if(self.homedatas[i].type == "animation"){
                ...
                _sp = C_setUI(self,size,self.homedatas[i]);
            }
            
            if(_sp != null && self.homedatas[i].animationtype == "righttoleft")
                self.sp_animation_righttoleft.push({"x":self.homedatas[i].x,"y":self.homedatas[i].y,"sp":_sp});
        }
...
//        });
        ...
        self.initTitleImageMoveTo();
        return true;
    },
     initTitleImageMoveTo:function(){
        var self = this;
        var size = cc.winSize;
        ...
        for(var i = 0 ; i < len; i ++){
            ...
            var delay = cc.delayTime(i*0.1);
            var seq = cc.Sequence.create(delay,move);
            sp.runAction(seq);
...
//                        text = (notice_date.getMonth()+1)+"/"+notice_date.getDate()+"  "+noticedata.lt_title;
                ...
                if(noticedata.lt_show == "Y")
                    self.sp_noticelist[i].setVisible(true);
                else 
                    self.sp_noticelist[i].setVisible(false);
            ...
            if(tag == self.homedatas[i].id){
                ...
                if(event){
                    if(event.page == "home" || event.page == "main"){
                        ...
                        console.log("팝업을 띄운다", );
                        var style = {"width": 1000, "height": 1000,"title":{"anchor":cc.TEXT_ALIGNMENT_CENTER,"fontsize":40},"message":{"anchor":cc.TEXT_ALIGNMENT_LEFT,"fontsize":28}};
                        var index = event.index ? event.index : 0;
                        var title =  C_WrapTextByWidth(notice_datas[index].lt_title, 25);
                    ...
                    else if(event.page == "popup_board" && !window.android){
