...
//        C_DrawBackColor(self);
    ...
    init:function(title,message, OKCallback , CancelCallback,style){
        ...
        self._bg.height = style && style.height ? style.height : 841;
//        self._bg.width = 1032;
//        self._bg.height = 616;
...
//        var message_fontsize = style && style.message && style.message.fontsize ? style.message.fontsize : 30;
        ...
        var pos = [
            {"x":self._bg.width/2-240,"y":530},
            {"x":self._bg.width/2,"y":530},
            {"x":self._bg.width/2+240,"y":530},
            {"x":self._bg.width/2-125,"y":300},
            {"x":self._bg.width/2+125,"y":300},
                  ]
        C_AddAnimButton(self._bg,res.n_home_language0,null,self._bg.width/2-240,530,language_dialog.language0,1,null,function(tag){
            self.select_idx = 0;            
            self.moveSelect();
        },self);
        C_AddAnimButton(self._bg,res.n_home_language1,null,self._bg.width/2,530,language_dialog.language1,1,null,function(tag){
            self.select_idx = 1;
            self.moveSelect();
        },self);
        C_AddAnimButton(self._bg,res.n_home_language2,null,self._bg.width/2+240,530,language_dialog.language2,1,null,function(tag){
            self.select_idx = 2;
            self.moveSelect();
        },self);
        C_AddAnimButton(self._bg,res.n_home_language3,null,self._bg.width/2-125,300,language_dialog.language0,1,null,function(tag){
            self.select_idx = 3;
            self.moveSelect();
        },self);
        C_AddAnimButton(self._bg,res.n_home_language4,null,self._bg.width/2+125,300,language_dialog.language1,1,null,function(tag){
            self.select_idx = 4;
            self.moveSelect();
        },self);
...
//        self._bg.addChild(message);
    ...
    getLanguageCode:function(){
         var self = this;
         var language_code = ["KO","EN","ZH","VI","MS"];
