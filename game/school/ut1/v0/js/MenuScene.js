...
//        C_Loading(self);
        ...
        self.now_sub = data.sub ? parseInt(data.sub) : 0;
        ///////////////////////////////////////
        //PDF 목록 테스트 START
        ///////////////////////////////////////
...
//        for(var i = 0 ; i < 3;i++){//test
        for(var i = 0 ; i < self.maindatas.length;i++){
            if(self.maindatas[i].type == "animation"){
                ...
                //네모 하단 말풍선 포인트
                if(self.maindatas[i].id == "home_tab")
                    self.tab_bg = _sp;
                if(self.maindatas[i].id == "home_tab_select")
                    self.tab_select = _sp;
...
//        self.setSubButtons(this._data);
    ...
    init:function(data){
        ...
        if(self.now_tab != 10){
            if(self.sp_title){
                self.sp_title.removeFromParent(true);
                self.sp_title = null;
            }
            self.sp_title = C_AddCustomFont(self, data.name, 235, C_getY(115), null, 1, cc.TEXT_ALIGNMENT_LEFT , 60, C_HEXtoCCColor("#20405b"),true);    
        }
        if(self.now_tab != 2){
            self.tab_bg.setVisible(false);
            self.tab_select.setVisible(false);
        }
...
//        }    
        ...
        if(self.now_contentdata){
             ...
             if(self.now_tab == 10){
                ...
                self.hideWebView();
                self.drawPoster();
            }else if(self.now_tab == 2){
                console.log("self.now_sub "+self.now_sub);
                 this.pageurl = self.now_contentdata.url;
               
                    self.tab_select.setPosition(108+self.now_sub*216,C_getY(300));    
...
//            console.log("textureSize width "+textureSize.width+" height "+textureSize.height);
            ...
            const scaleY = 1920 / textureSize.height;
 ...
 before_touchx : -1, //터치시 홈화면으로 이동  START
        ...
        if(schoolapitype && useschoolapi == 1){
            ...
            switch(type){
                ...
                case "board":
                    ...
                    break;
                case "pdf_gallery":
                    fname = "../../../webpage/type_pdf_gallery.html";
                    break;
                case "gallery":
    ...
    addWebView: function(parent, x, y, w, h, pageurl) {
        ...
        var _ts = "&_ts=" + Date.now();
        console.log("_pdfpagetype is "+_pdfpagetype);
        const url = pageurl+"?projectid="+param_projectid+"&groupidx="+param_groupidx+"&listid="+self.now_contentdata.id+_param_schoolapitype+_param_gamemode+_param_surveyidx+_param_surveyonoff+_param_pdf_category+_pdfpagetype+_ts;
...
//        this.addWebView(this.sp_rect, 1200+mx, 550+my, 1400, 787,  this.pageurl);//16:9  // 일반 비율
        ...
        if(istab){
            this.sp_rect.setPosition(540,890);
            this.addWebView(this.sp_rect, 540+mx, 790+my, 1040, 1550,  this.pageurl);//4:3 power point 비율
        }
        else 
            this.addWebView(this.sp_rect, 540+mx, 820+my, 1040, 1600,  this.pageurl);//4:3 power point 비율
    },
                ...
                if(tag == self.maindatas[i].id){
                    ...
                    if(event){
                        if(event.page == "home"){
                            ...
                            var data = event;           
                            data.id = self.maindatas[i].id;
                            data.name = self.maindatas[i].text ? self.maindatas[i].text[self.language_code].message.replace("|"," ") : self.maindatas[i].name;
                            C_nextScene(scene, data);                                 
                        }
                        else  if(event.page == "main"){
                             var scene = event.page == "main" ? SCENE_MENU : SCENE_HOME;
                            var data = event;           
                            data.id = self.maindatas[i].id;
                            data.name = self.maindatas[i].text ? self.maindatas[i].text[self.language_code].message.replace("|"," ") : self.maindatas[i].name;
                            console.log("data is ",data)
                            this._data = data;
                            self.init(this._data);
                        }else if(event.page == "reload"){
                            if(self.webViewPage == "" || self._data.id != "home_btn11" && self.webViewPage == "list0" ){
                                C_nextScene(SCENE_HOME, event);     
                            }
                            else {
                                self._data.webViewPage = self.webViewPage;
                                C_nextScene(SCENE_MENU, self._data);     
                            }
...
//                                 self.showWebView();
        ...
        //학교공지사항일때만 들어온다.
        else if(checkType(tag) == "int"){
            var _tab = 2;
            var _sub = tag
            console.log("_tab "+_tab+" _sub "+_sub);
            this._data.tab = _tab;
            this._data.sub = _sub;
            C_nextScene(SCENE_MENU,this._data);
