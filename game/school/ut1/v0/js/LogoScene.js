...
//                    self.sp_bg_building.runAction(move);
        ...
        if(notice_datas.length == 0){
            C_getListData("noticelist", 1, 5, function(noticelist){
                
                ////////////////////////////////////////
                // 공지사항 상단고정 체크하여 정렬하기 START
                ////////////////////////////////////////
                 ...
                 for(var i = 0 ; i < noticelist.length; i++){
                     ...
                     if(data.lt_istop && data.lt_istop == "Y"){
                         _toplist.push(data);
                     }else{
                         _list.push(data);
                     }
                 ...
                 for(var i = 0; i < _toplist.length; i++){
                     notice_datas.push(_toplist[i]);
                 }
                 for(var i = 0; i < _list.length; i++){
                     notice_datas.push(_list[i]);
                 }
                ////////////////////////////////////////
                // 공지사항 상단고정 체크하여 정렬하기 END
                ////////////////////////////////////////
    ...
    initNoticeList:function(_isShowMd){
        ...
        for(var i = 0 ; i < 5; i++){
            ...
            if(notice_datas.length > i){
                ...
                self.sp_noticelist[i].string = C_MaxLen(extractPlainText(text), self.TITLE_MAX_LEN);
                if(notice_datas[i].lt_istop && notice_datas[i].lt_istop == "Y")self.sp_noticelist[i].setColor(C_HEXtoCCColor("#6a0000"));
               ...
               var event = self.homedatas[i].event ? self.homedatas[i].event : null; 
                ...
                if(event){
                    if(event.page == "home" || event.page == "main"){
                        ...
                        var titledata = C_WrapTextByWidth(extractPlainText(notice_datas[index].lt_title), self.TITLE_MAX_LEN+10); 
                        var title =  titledata.text;
                        var data = C_WrapTextByWidth(extractPlainText(notice_datas[index].lt_message), self.MAX_LEN);
                        var message = data.text;
                        var lineCount = data.line;
                        
                        var height = lineCount < 10 ? 600 : 100+lineCount*40;
                        var style = {
                            height: height
                        };
...
//                        },null,style);
                        ...
                        C_ShowWebViewDialogLayer(self,title, notice_datas[index].lt_message, 1000,800, function(){

                             console.log("closeMapdialogLayer 111");
//                            location.reload();
                        },null,{"bgres": res._global_img_ninepatch_white_bg});   
//                          C_nextScene(SCENE_MENU, {"page":"main","tab":1,"sub":0,  "id": "noticelist", "url": "contentdata/page1_0.html", "name":"전체공지보기"});   
//                        C_nextScene(SCENE_GAME, {"page":"main","tab":1,"sub":0,  "id": "noticelist", "url": "contentdata/page1_0.html", "name":"전체공지보기"});   
                    }
                        ...
                        C_ShowWebViewDialogLayer(self,"공지사항 목록", self.noticeboard_event, 900,1200, function(){
