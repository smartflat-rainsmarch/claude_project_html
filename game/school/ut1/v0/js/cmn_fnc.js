...
function C_AddCustomFont(parent, str, x, y, tag, scale, anchor, fontsize, color, isboldfont, onClick) {
    ...
    str = extractPlainText(str);
    
     var labelFont = isboldfont ? "현대하모니B" : "현대하모니M";
    var label = null;
...
function C_WrapTextByWidth(str, maxCharsPerLine) {
    ...
    return {"text":result, "line":linecount};
}
...
function drawAirText(parent, x, y, mode, dustInfo){
   ...
   if(!parent)return;
    ...
    if(mode == "icon"){
        ...
        C_AddCustomFont(parent,"미세먼지", x, y+30, null, 1, cc.TEXT_ALIGNMENT_CENTER , 50, C_HEXtoCCColor("#313131"),true);    
        if(_icon)C_AddSprite(parent,_icon, x, y-25,1,1,null);    
    }else { // text mode
        var _grade = dustInfo.grade ? dustInfo.grade : "";
        C_AddCustomFont(parent,"미세먼지 ", x, y+30, null, 1, cc.TEXT_ALIGNMENT_CENTER , 50, C_HEXtoCCColor("#313131"),true);    
        C_AddCustomFont(parent,""+_grade, x, y-30, null, 1, cc.TEXT_ALIGNMENT_CENTER , 50, C_HEXtoCCColor("#313131"),true);    
...
function drawWeatherToday(parent, x, y, weatherdata, spdata){//mode, _fontColor, _fontSizeBig, _fontSizeSmall,iconscale){
     ...
     C_AddCustomFont(parent,weatherdata.today_tmp+"℃", x-40, y+25, null, 1, cc.TEXT_ALIGNMENT_CENTER , spdata.fontsize_big, C_HEXtoCCColor(spdata.fontcolor),false);    
     C_AddSprite(parent,getWeatherIconRes(weatherdata.today_sky, weatherdata.today_pty), x+80, y+30,1,spdata.iconscale,null);    
     C_AddCustomFont(parent,"최저"+weatherdata.today_tmn+"℃ / 최고"+weatherdata.today_tmx+"℃", x+10, y-35, null, 1, cc.TEXT_ALIGNMENT_CENTER , spdata.fontsize2_small, C_HEXtoCCColor(spdata.fontcolor),false);    
...
/**
 * HTML 태그가 포함된 문자열에서 모든 태그를 제거하고 순수 텍스트만 추출하는 함수
 * @param {string} htmlString - HTML 태그가 포함된 원본 문자열 (예: "<b>Hello</b> <br/> World")
 * @returns {string} - 태그가 제거된 순수 텍스트 문자열 (예: "Hello  World")
 */
function extractPlainText(htmlString) {
    ...
    // 2. [단계 1] 스크립트(<script>) 및 스타일(<style>) 태그 내용 통째로 제거
    text = text.replace(/<script\b[^>]*>([\s\S]*?)<\/script>/gim, '');
    text = text.replace(/<style\b[^>]*>([\s\S]*?)<\/style>/gim, '');

    // 3. [단계 2] <br> 태그 예외 처리 (핵심!)
    // <br>, <br/>, <br  > 등 다양한 형태를 대소문자 구분 없이 \n 으로 치환
    text = text.replace(/<br\s*\/?>/gi, '\n');

    // 4. [단계 3] &nbsp; 엔티티 예외 처리 (핵심!)
    // &nbsp;를 대소문자 구분 없이 일반 공백 ' ' 으로 치환
    text = text.replace(/&nbsp;/gi, ' ');

    // 5. [단계 4] 나머지 모든 HTML 태그 제거 (<div>, <b> 등)
    // < 로 시작해서 > 로 끝나는 모든 패턴을 찾음
    var tagRegex = /<[^>]*>/gi;
    text = text.replace(tagRegex, '');

    // 6. [단계 5] HTML 엔티티(&lt;, &gt; 등) 처리
    // 예외 처리하지 않은 다른 엔티티들을 실제 문자로 변환
    text = text.replace(/&lt;/gi, '<');
    text = text.replace(/&gt;/gi, '>');
    text = text.replace(/&amp;/gi, '&');
    text = text.replace(/&quot;/gi, '"');
    ...
    // 7. [단계 6] 불필요한 공백 및 줄바꿈 정리 (선택 사항)
    // 태그가 제거되면서 생긴 연속된 공백이나 줄바꿈을 정리합니다.
    
    // 연속된 공백(스페이스, 탭 등) -> 하나의 공백 ' ' 으로 치환
    // 줄바꿈 문자(\n)는 제외하고 처리
    text = text.replace(/[ \t]+/g, ' ');
...
//해당 컨텐츠에서만 애니메이션을 만든다.
function initEffectAnimation(self){
        
    var size = cc.winSize;
    self.sp_effect = C_AddSprite(self, "res/n_home/effect.png", -100, C_getY(205), 1, 1, null);

    
    var move = cc.moveTo(0,-100,C_getY(205));
    var move0 = cc.moveTo(1,680,C_getY(205));
    var rotate0 = cc.rotateBy(0.01,-45,0);
    var move1 = cc.moveTo(0.5, 820, C_getY(55));
    var rotate1 = cc.rotateBy(0.01,45,0);
    var move2 = cc.moveTo(0.5, 1200, C_getY(55));
    var delay = cc.delayTime(5);

    
    var amove = cc.moveTo(0,-100,C_getY(205));
    var amove0 = cc.moveTo(1,680,C_getY(205));
    var arotate0 = cc.rotateBy(0.5,-1125,0);
    var amove1 = cc.moveTo(0.5, 820, C_getY(55));
    var arotate1 = cc.rotateBy(0.5,1125,0);
    var amove2 = cc.moveTo(0.5, 1200, C_getY(55));
    var adelay = cc.delayTime(5);


    var seq = cc.sequence(move,move0,rotate0,move1,rotate1,move2,delay,amove,amove0,arotate0,amove1,arotate1,amove2,adelay);
     var repeat = C_Repeat_Forever(seq);
    self.sp_effect.runAction(repeat);
