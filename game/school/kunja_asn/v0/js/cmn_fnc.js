...
function C_AddTextButton(parent, res, x, y, tag, scale, zindex, callback, text, textsize, hex_textcolor, istextbold) {
    var sp = C_AddSpriteTouch(parent, res, x, y, tag, scale, zindex, callback);
...
function C_AddSpriteTouch(parent, res, x, y, tag, scale, zindex, callback) {
...
function C_ShowDialogLayer(self, title , message, OKCallback , CancelCallback,style, closetime){
    var layer = new DialogLayer();
    layer.setTag(ID.POPUP_LAYER_DIALOG);
    self.addChild(layer, 2222);
    layer.init(title,message,OKCallback, CancelCallback,style, closetime);
...
function C_WrapTextByWidth(str, maxCharsPerLine) {
    ...
    for (let i = 0; i < str.length; i++) {
        result += str[i];
        // 줄바꿈 삽입
        if ((i + 1) % maxCharsPerLine === 0 && i !== str.length - 1) {
            result += '\n';
        }
    }
    return result;
...
function drawWeatherToday(parent, x, y, weatherdata, spdata){//mode, _fontColor, _fontSizeBig, _fontSizeSmall,iconscale){
     ...
     C_AddCustomFont(parent,weatherdata.today_tmp+"℃", x, y+20, null, 1, cc.TEXT_ALIGNMENT_CENTER , 60, C_HEXtoCCColor(spdata.fontcolor),false);    
     C_AddSprite(parent,getWeatherIconRes(weatherdata.today_sky, weatherdata.today_pty), x+150, y+40,1,spdata.iconscale,null);    
...
function getBGIconRes( sp_bg, sky, pty ) {
   ...
   if(pty > 0) {
    ...
    var currentTime = hour * 100 + minute; // HHMM 형태 정수 (예: 930 = 9:30)

    // 맑음/부분 맑음 계열이면 시간대별로 아이콘 변경
    if (iconName === 'sunny' || iconName === 'partlysunny' || iconName === 'partlycloudy') {
        if (currentTime >= 600 && currentTime <= 1000) {
            iconName = 'morning';   // 오전 6시~10시
        } else if (currentTime > 1000 && currentTime <= 1400) {
            iconName = 'sunny';     // 오전 10시~오후 2시
        } else if (currentTime > 1400 && currentTime <= 1600) {
            iconName = 'afternoon'; // 오후 2시~4시
        } else if (currentTime > 1600 || currentTime < 600) {
            iconName = 'dinner';    // 오후 4시 이후~다음날 오전 6시
        }
    }
    if (currentTime > 1600 || currentTime < 600)
            iconName = 'dinner';    // 오후 4시 이후~다음날 오전 6시
    console.log("./res/n_home/mbg_" + iconName + ".png");
    
    if(iconName == "sunny" || iconName == "partlysunny") 
        drawLightAni(sp_bg);        
    else if(iconName == "snow" )
        drawSnowParticle(sp_bg);
    else if(iconName == "rain" || iconName == "sleet")
        drawRainParticle(sp_bg);
    
    
//    iconName = "afternoon";//test
   
   return iconName; //"./res/n_home/mbg_" + iconName + ".png";
};
function drawLightAni(parent){
    console.log("빛을 보여준다");
    if (!parent) {
        cc.log('drawLightAni: parent is null');
        return;
    }
    var anchor = parent.getAnchorPoint();   // {x:0.5, y:0.5} 이런 식으로 나옴
    console.log("parent anchor: ", anchor.x, anchor.y);
    var claim = C_AddSprite(parent, res.n_effect_claim, 0, C_getY(0), 1, 5, null);
    
    var time0 = 9;
    var rotate0 = cc.rotateBy(time0, 10, 10);
    var scale0 = cc.scaleTo(time0, 6.0);
    var spawn0 = cc.spawn(scale0, rotate0);
    
    var time1 = 8;
    var rotate1 = cc.rotateBy(time1, -10, -10);
    var scale1 = cc.scaleTo(time1, 10.0);
    var spawn1 = cc.spawn(scale1, rotate1);
    var seq = cc.sequence(spawn0, spawn1);
    var repeatForever = cc.repeatForever(seq);
    claim.runAction(repeatForever);
    
   
}
function drawSnowParticle(parent) {
    if (!parent) {
        cc.log('drawSnowParticle: parent is null');
        return;
    }

    // winSize 가져오기 (버전 차이 보정)
    var winSize = (cc.director && cc.director.getWinSize) ? cc.director.getWinSize() : cc.winSize;

    try {
        var emitter = null;

        // 1) 가능하면 내장된 ParticleSnow 사용 (외부 텍스처 불필요)
        if (typeof cc.ParticleSnow !== 'undefined') {
            emitter = new cc.ParticleSnow();

            // 기본 값들 조정
            emitter.totalParticles = 500;
            emitter.duration = cc.ParticleSystem.DURATION_INFINITY;
            emitter.setPosition(winSize.width / 2, winSize.height + 10);
            emitter.posVar = cc.p(winSize.width / 2, 0);
            emitter.gravity = cc.p(0, -30);
            emitter.speed = 30;
            emitter.life = 18;
            emitter.lifeVar = 15;
            emitter.startSize = 32;
            emitter.startSizeVar = 6;
            emitter.setBlendAdditive(false);
            
               // 좌우로 살짝 흔들리는 효과
            emitter.tangentialAccel = 15;    // 옆으로 휘는 힘
            emitter.tangentialAccelVar = 30; // 가속도 변화량

            emitter.angle = 90;     // 기본 각도 (아래 방향)
            emitter.angleVar = 20;  // 좌우로 퍼지는 각도

        // 2) 없으면 ParticleSystemQuad로 직접 세팅 (텍스처는 선택)
        } 
        // 부모에 추가 (z-order 높게)
        parent.addChild(emitter, 9999);
        cc.log('drawSnowParticle: emitter added');

    } catch (err) {
        cc.log('drawSnowParticle error: ' + err);
    }
}
function drawRainParticle(parent) {
    if (!parent) {
        cc.log('drawSnowParticle: parent is null');
        return;
    }
    ...
    try {
        ...
        // 1) 가능하면 내장된 ParticleSnow 사용 (외부 텍스처 불필요)
        if (typeof cc.ParticleRain !== 'undefined') {
            emitter = new cc.ParticleRain();

            // 기본 값들 조정
            emitter.totalParticles = 1500;
            emitter.duration = cc.ParticleSystem.DURATION_INFINITY;
            emitter.setPosition(winSize.width / 2, winSize.height + 10);
            emitter.posVar = cc.p(winSize.width / 2, 0);
            emitter.gravity = cc.p(10, -5630);
            emitter.speed = 5630;
            emitter.life = 1.2;
            emitter.lifeVar = 0.3;
            emitter.startSize = 20;   // 높이
            emitter.startSizeVar = 20;
            emitter.endSize = 20;     // 끝날 때도 동일 크기
            emitter.endSizeVar = 20;
            emitter.startColor = cc.color(255, 255, 255, 255);
            emitter.endColor = cc.color(255, 255, 255, 0);
            emitter.setBlendAdditive(false);            
        } 
...
//            }, self);
        ...
        case DATATYPE.BGCOLOR:
         ...
         case DATATYPE.WEATHERAPI:
            ...
            C_updateWeather(sp , data, self.obj_bg);
            ...
            if (data.texttype) {
                ...
                switch (data.texttype) {
                    case "m/d":
                        ...
                        mtext = (date.getMonth() + 1) + "/" + date.getDate();
                        break;
                    case "hh:mm":
                        // 시:분
...
//                        mtext = hour + ":" + minute;
                        ...
                        var ampm = hh24 >= 12 ? "오후" : "오전";   // 오전/오후 구분
...
//        hhmm.string = hh+":"+mm;
        ...
        var weekdays = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
        weekday.string = weekdays[date.getDay()];                
    }
}
...
//1시간마다 날씨, 미세먼지 업데이트
function C_updateWeatherAirBG(obj_weather, obj_air, obj_bg, nowtime){
...
//     }
     ...
     var date = new Date();
    var h = date.getHours();
    if(before_weatherhour == -1 || before_weatherhour != h)C_updateWeather(obj_weather.sp, obj_weather.data, obj_bg);
...
var now_weatherdata = {};
function C_updateWeather(sp , data, bgimg){
   ...
   if(before_weatherhour == -1 || before_weatherhour != h){
       ...
       getWeatherData(data.mode, function(weatherdata_str){
           ...
           C_updateBG(bgimg,now_weatherdata);
...
function C_updateBG(sp_bg, weatherdata, nowtime) {
    var bgInterval = setInterval(function(){
        if (sp_bg && sp_bg.getTexture && sp_bg.getTexture()) {
            if(sp_bg != null && weatherdata != null){        
                const iconName = getBGIconRes(sp_bg, weatherdata.today_sky, weatherdata.today_pty);
                const newImagePath = "./res/n_home/mbg_" + iconName + ".png"
                // sp_bg 요소의 src 속성을 새로운 이미지 경로로 설정합니다.
                if(newImagePath){
                    sp_bg.setTexture(newImagePath);


                    
                }
                console.log("배경 이미지 업데이트됨: " + newImagePath);
...
function C_updateAir(sp, data){
   ...
   if(before_airhour == -1 || before_airhour != h){
        ...
        //            console.log(" item is ",item);
                    ...
                    drawAirText(sp, 0, 0, data.mode, dustInfo);
                    drawProgressArc(sp, 150, 15,  item.pm10Value, data.max, data.radius, data.fontcolor, data.fontsize, data.arclinewidth); 
                }
            }
        });
   }else {
