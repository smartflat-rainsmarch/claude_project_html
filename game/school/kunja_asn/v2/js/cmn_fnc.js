...
function getDustLevelInfo(pm10, pm25) {
  ...
  return {
    grade: levels[finalGrade].grade,
    iconClass: levels[finalGrade].icon
  };
}
...
function drawAirText(parent, x, y, mode, dustInfo){
   ...
   if(!parent)return;
    ...
    if(mode == "icon"){
        ...
        C_AddSprite(parent,dustInfo.iconClass, x, y-30,1,1,null);    
    }else { // text mode
        C_AddCustomFont(parent,"미세먼지", x, y+30, null, 1, cc.TEXT_ALIGNMENT_CENTER , 40, C_HEXtoCCColor("#ffffff"),false);    
        C_AddCustomFont(parent,dustInfo.grade, x, y-20, null, 1, cc.TEXT_ALIGNMENT_CENTER , 35, C_HEXtoCCColor("#ffffff"),false);    
    }
        ...
        if (sp_bg && sp_bg.getTexture && sp_bg.getTexture()) {
            if(sp_bg != null && weatherdata != null){        
                ...
                if(newImagePath){
                    ...
                    if(iconName == "sunny" || iconName == "partlysunny") drawLightAni(sp_bg);        
                    else if(iconName == "snow" )drawSnowParticle(sp_bg);
                    else if(iconName == "rain" || iconName == "sleet")drawRainParticle(sp_bg);
                }
