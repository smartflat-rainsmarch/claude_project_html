...
var LogoLayer = cc.Layer.extend({
    ...
    obj_bg : null, //이게 null 이 아니라면 배경이 날씨에 따라 바뀐다.
    ctor:function () {
        //////////////////////////////
        // 1. super init first
        this._super();
...
//        this.addChild(new cc.LayerColor.create(cc.color(255, 255, 255, 255)));
        ...
        for(var i = 0 ; i < self.homedatas.length;i++){
            ...
            else if(self.homedatas[i].type == "bgimg"){
                //이게 null 이 아니라면 배경이 날씨에 따라 바뀐다.
                self.obj_bg = C_setUI(self,size,self.homedatas[i]);
                bgimgdata = self.homedatas[i];
            }
            else {
                C_setUI(self,size,self.homedatas[i]);
            }
...
//        });
    ...
    update: function (dt) {
        ...
        C_updateWeatherAirBG(self.obj_weather, self.obj_air, self.obj_bg, this.wtime);//날씨 , 미세먼지 업데이트
        C_checkAutoUpdate();
