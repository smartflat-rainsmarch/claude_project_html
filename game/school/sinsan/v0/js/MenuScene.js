...
var MenuLayer = cc.Layer.extend({
    ...
    sp_animation_scale: [], //여기에 담기면 탭별로 이미지가 담아야한다.
    now_tab:-1,
    indicator_on : [],
    isshowPopup:false,//팝업 어뷰징을 막기위해
    now_imagefoldername: "",
    orientation:"L",
    ctor: function (data) {
        //////////////////////////////
...
//        this.addChild(new cc.LayerColor.create(cc.color(255, 255, 255, 255)));
        ...
        self.orientation =  self.homepage_data.hm_orientation;
        
        if(parseInt(data.tab) == 0){
            self.now_imagefoldername = "gallery_"+data.sub;
        }
...
//        this.addChild(new cc.LayerColor.create(cc.color(255, 0, 0, 77)));
    ...
    initIndecator : function(indicator){
        ...
        for(var i = 0;i < len; i++){            
            ...
            this.indicator_on.push(C_AddSprite(self,res.indicator_on, indicator.x-offset+i*40, C_getY(indicator.y),1,1.7,null));            
        }
...
//            console.log("textureSize width "+textureSize.width+" height "+textureSize.height);
            const scaleX = 1920 / textureSize.width;
            const scaleY = 1080 / textureSize.height;
    ...
    addWebView: function(parent, x, y, w, h, pageurl) {
        ...
        var _param_cb = "&cb="+((new Date()).getTime());//계속갱신
        ...
        var _param_imagefoldername = self.now_imagefoldername ? "&imgfoldername="+self.now_imagefoldername : "";
        var _param_orientation =  "&orientation="+self.orientation;
        var _param_imgh = self.now_contentdata.type == "gallery1" ? "&imgh=700px" : "";
        var _param_isinfo = self.now_contentdata.type == "gallery1" ? "&isinfo=1" : "";
        this.webview.loadURL(pageurl+"?projectid="+param_projectid+"&groupidx="+param_groupidx+"&listid="+self.now_contentdata.id+_param_schoolapitype+_param_gamemode+_param_surveyidx+_param_surveyonoff+_param_imgh+_param_isinfo+_param_orientation+_param_imagefoldername+_param_cb);
...
//        console.log("this.pageurl ",this.pageurl);
    ...
    showWebView: function() {
       var self = this;
        var size = cc.winSize;
        console.log("showWebView!! ",this.pageurl);
         // 실제 브라우저 화면 크기
        var screen_width = $(window).width();
        var screen_height = $(window).height();
...
//        console.log("screen_height "+screen_height+" canvas_height "+(canvas_height*finalScale));
        ...
        this.addWebView(this, mx+960, my+518, 1720, 890, this.pageurl);
//        this.addWebView(this.sp_rect, mx, my, 1734, 894, this.pageurl);
    ...
    setTabButtons:function(data){
         var self = this;
         var size = cc.winSize;
        const tab_width = [0,1020,510,340,258,216,170,145,127,113];
         var _tab = data.tab;
         var _sub = data.sub;
                        ...
                        if(event.page == "home" || event.page == "main"){
                            ...
                            if(!this.isshowPopup){
                                ...
                                C_ShowMapDialogLayer(self,"","",mapdatas, function(){
                                 ...
                                 //재단축제 동영상이거나 , 재단 축제 이미지 이면 웹뷰를 보여준다.
                                 if(self.now_tab == 3 || self.now_tab == 4)
                                     self.showWebView();
...
//            console.log("_tab "+_tab+" _sub "+_sub);
            ...
            if(_tab != self.now_tab && diff >= 1000){
