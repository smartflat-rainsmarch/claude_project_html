...
function C_ChangeSpriteImage(targetSp, newRes) {
    ...
    // 1. 기존 캐시 제거 (새 이미지를 강제로 반영하기 위함)
    cc.textureCache.removeTextureForKey(newRes);

    // 2. 비동기로 새로운 텍스처 로드 후 적용
    cc.textureCache.addImage(res_url, function(texture) {
        if (texture) {
            targetSp.setTexture(texture);
            
            // 만약 이미지 크기에 맞춰 스프라이트 크기를 재조정해야 한다면 아래 코드 추가
            // var rect = cc.rect(0, 0, texture.width, texture.height);
            // targetSp.setTextureRect(rect);
            
            cc.log("이미지 교체 완료: " + newRes);
        }
    }, this);
}
...
function C_Loading(self,color,delaytime){
    var _delaytime = delaytime ? delaytime : 600;
...
function C_ShowDialogLayer(self, title , message, OKCallback , CancelCallback,style){
    ...
    layer.setTag(ID.POPUP_LAYER_DIALOG);
    self.addChild(layer, 2222);
    layer.init(title,message,null, OKCallback, CancelCallback,style);
...
var currentListMax = 100;
var currentListId = "";
var now_pagenum = 1;
var alllist = [];
//자동업데이트체크
function getReservationList(pagenum, list_length, list_name_id, callback) {
    currentListMax   = list_length;
    currentListId    = list_name_id;
    currentProjectId = param_projectid;
    now_pagenum = pagenum;
    const _data = {
        "projectid": param_projectid,
        "groupidx":param_groupidx,
        "listnameid": currentListId,
        "pagenum": pagenum,
        "listmax": currentListMax//
    };
...
//        
//        C_AddCustomFont(parent,"미세먼지", x, y+35, null, 1, cc.TEXT_ALIGNMENT_CENTER , 20, C_HEXtoCCColor("#ffffff"),false);    
//        if(_icon)C_AddSprite(parent,_icon, x, y-35,1,1,null);    
//    }else { // text mode
//        var _grade = dustInfo.grade ? dustInfo.grade : "";
//        C_AddCustomFont(parent,"미세먼지 "+_grade, x, y+35, null, 1, cc.TEXT_ALIGNMENT_CENTER , 20, C_HEXtoCCColor("#ffffff"),false);    
...
//     C_AddCustomFont(parent,weatherdata.today_tmp+"℃", x, y+20, null, 1, cc.TEXT_ALIGNMENT_CENTER , 60, C_HEXtoCCColor(spdata.fontcolor),false);    
     if(weatherdata.today_sky && weatherdata.today_pty)C_AddSprite(parent,getWeatherIconRes(weatherdata.today_sky, weatherdata.today_pty), x-75, y+7,1,spdata.iconscale,null);    
//     C_AddCustomFont(parent,"최저"+weatherdata.today_tmn+"℃", x, y, null, 1, cc.TEXT_ALIGNMENT_CENTER , 18, C_HEXtoCCColor(spdata.fontcolor),false);    
     if(weatherdata.today_tmn && weatherdata.today_tmx)C_AddCustomFont(parent,weatherdata.today_tmn+"℃/"+weatherdata.today_tmx+"℃", x, y, null, 1, cc.TEXT_ALIGNMENT_CENTER , 18, C_HEXtoCCColor(spdata.fontcolor),false);    
...
//            }, self);
        ...
        case DATATYPE.TEXT:
            var mtext = data.text;
            var isboldfont = data.fontweight && data.fontweight == "bold" ? true : false;
            if (data.texttype) {
                var date = new Date();
...
//                        mtext = hour + ":" + minute;
            ...
            sp = C_AddCustomFont(self,mtext, parseInt(data.x), C_getY(parseInt(data.y)), data.id, 1, text_align ,data.fontsize, C_HEXtoCCColor(data.fontcolor),isboldfont,function(tag){
                 self.buttonClick(tag);
            });
            break;
    }
    ...
    self.all_id_sps.push({"imgurl":C_getResPath(data.imgurl), "sp":sp});
    return sp;
}
...
var now_weatherdata = null;
function C_updateWeather(sp , data){
    sp.removeAllChildren();
    var date = new Date();
    var h = date.getHours();
   if(now_weatherdata == null || before_weatherhour == -1 || before_weatherhour != h){
...
//         const posX = (areaWidth-itemWidth)/2;
        const posY = contentHeight - (y * (itemHeight + gapY)) - 180;
