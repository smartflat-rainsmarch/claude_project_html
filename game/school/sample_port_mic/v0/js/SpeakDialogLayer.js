...
//        this.addChild (self._bg);
    ...
    if (this.isspeak === 2) {
        ...
        this.speak_ani = C_spriteAnimation(self._bg, size.width/2, C_getY(1350), arr_flag, 0.1, 1, function() {
            ...
            stopMediaRecord(param_projectid, param_groupidx, null, function(res) {
                console.log("음성 답변 데이터 ", res);
                self.txt_message.string = "";
                var _question = res.question ? res.question : "";
                var _answer = res.answer ? res.answer : "";
                var _tab = res.tab ? res.tab : -1;
                var _sub = res.sub != undefined ? res.sub : 0;
