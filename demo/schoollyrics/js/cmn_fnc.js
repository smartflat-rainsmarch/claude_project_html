...
function C_LeftRightRepeatAni(sp,time){
       sp.setPosition(0,sp.getPosition().y);
        var move1 = cc.moveBy(time, 1080,0);
        var move2 = cc.moveBy(time,-1080,0);
        var seq1 = cc.sequence(move1,move2);
        var repeat1 = C_Repeat_Forever(seq1);//cc.RepeatForever.create(seq1);
        sp.runAction(repeat1);
}
...
function C_setUI(self,size,data){
    ...
    switch(data.type){
        ...
        case DATATYPE.VIDEO:
            ...
            if(data.w < 1080)backgroundVideo.style.width = data.w+"px";
            if(data.h < 1920)backgroundVideo.style.height = data.h+"px";
...
function C_updateAir(sp, data){
   ...
   if(before_airhour == -1 || before_airhour != h){
    ...
    //            console.log(" item is ",item);
                ...
                drawProgressArc(sp, 150, 5,  item.pm10Value, data.max, data.radius, data.fontcolor, data.fontsize, data.arclinewidth); 
            }
        });
   }else {
       if(now_airdata.response && now_airdata.response.body && now_airdata.response.body.items && now_airdata.response.body.items.length > 0){
...
function C_PlayBrickBreakEffect(parentNode, x, y) {
    ...
    // 2. 파티클 속성 설정
    particle.setTexture(cc.textureCache.addImage("res/n_home/block.png")); // 파편 텍스처 이미지
...
function saveRanking(nickname, score, rank){
    var new_rank = [];
...
function submitNickname() {
    ...
    if (window._pendingRanking) {
        ...
        saveRanking(nickname, score, rank);
...
function updateRankingData(time) {
    ...
    showInputRankingPopup(time, rank);
    return rank;
}
/////////////////////////////////////////
//교가 랭킹 입력팝업  END
